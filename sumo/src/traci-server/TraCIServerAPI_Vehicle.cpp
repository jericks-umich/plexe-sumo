...
bool TraCIServerAPI_Vehicle::processGet(TraCIServer &server,
  ...
  // check variable
  if (variable != ID_LIST && variable != VAR_SPEED &&
      variable != VAR_SPEED_WITHOUT_TRACI && variable != VAR_POSITION &&
      variable != VAR_ANGLE && variable != VAR_POSITION3D &&
      variable != VAR_ROAD_ID && variable != VAR_LANE_ID &&
      variable != VAR_LANE_INDEX && variable != VAR_TYPE &&
      variable != VAR_ROUTE_ID && variable != VAR_COLOR &&
      variable != VAR_LANEPOSITION && variable != VAR_CO2EMISSION &&
      variable != VAR_COEMISSION && variable != VAR_HCEMISSION &&
      variable != VAR_PMXEMISSION && variable != VAR_NOXEMISSION &&
      variable != VAR_FUELCONSUMPTION && variable != VAR_NOISEEMISSION &&
      ...
      variable != VAR_LENGTH && variable != VAR_MAXSPEED &&
      variable != VAR_VEHICLECLASS && variable != VAR_SPEED_FACTOR &&
      variable != VAR_SPEED_DEVIATION && variable != VAR_ALLOWED_SPEED &&
      variable != VAR_EMISSIONCLASS && variable != VAR_WIDTH &&
      variable != VAR_MINGAP && variable != VAR_SHAPECLASS &&
      variable != VAR_ACCEL && variable != VAR_DECEL &&
      variable != VAR_IMPERFECTION && variable != VAR_TAU &&
      variable != VAR_BEST_LANES && variable != DISTANCE_REQUEST &&
      variable != ID_COUNT && variable != VAR_STOPSTATE &&
      variable != VAR_WAITING_TIME && variable != VAR_ROUTE_INDEX &&
      variable != VAR_PARAMETER && variable != VAR_GET_SPEED_AND_ACCELERATION &&
      variable != VAR_GET_LANES_COUNT && variable != VAR_GET_CC_INSTALLED &&
      variable != VAR_GET_RADAR_DATA && variable != VAR_GET_DISTANCE_TO_END &&
      variable != VAR_GET_LANE_CHANGE_ACTION &&
      variable != VAR_GET_ACTIVE_CONTROLLER &&
      variable != VAR_GET_DISTANCE_FROM_BEGIN && variable != VAR_GET_CRASHED &&
      variable != VAR_GET_ACC_ACCELERATION &&
      variable != VAR_GET_CACC_SPACING &&
      variable != VAR_GET_GENERIC_INFORMATION) {
    return server.writeErrorStatusCmd(
        CMD_GET_VEHICLE_VARIABLE,
        "Get Vehicle Variable: unsupported variable " + toHex(variable, 2) +
            " specified",
        outputStorage);
  }
  ...
  if (variable == ID_LIST || variable == ID_COUNT) {
    ...
    SUMOVehicle *sumoVehicle =
        MSNet::getInstance()->getVehicleControl().getVehicle(id);
    if (sumoVehicle == 0) {
      return server.writeErrorStatusCmd(CMD_GET_VEHICLE_VARIABLE,
                                        "Vehicle '" + id + "' is not known",
                                        outputStorage);
    }
    ...
    if (v == 0) {
      return server.writeErrorStatusCmd(
          CMD_GET_VEHICLE_VARIABLE,
          "Vehicle '" + id + "' is not a micro-simulation vehicle",
          outputStorage);
    }
    ...
    case VAR_SPEED_WITHOUT_TRACI:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? v->getSpeedWithoutTraciInfluence()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_POSITION:
      tempMsg.writeUnsignedByte(POSITION_2D);
      tempMsg.writeDouble(visible ? v->getPosition().x()
                                  : INVALID_DOUBLE_VALUE);
      tempMsg.writeDouble(visible ? v->getPosition().y()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_POSITION3D:
      ...
      tempMsg.writeDouble(visible ? v->getPosition().y()
                                  : INVALID_DOUBLE_VALUE);
      tempMsg.writeDouble(visible ? v->getPosition().z()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_ANGLE:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? GeomHelper::naviDegree(v->getAngle())
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_ROAD_ID:
    ...
    case VAR_LANE_INDEX:
      ...
      if (onRoad) {
        ...
        tempMsg.writeInt((int)std::distance(
            lanes.begin(),
            std::find(lanes.begin(), lanes.end(), v->getLane())));
      } else {
    ...
    case VAR_LANEPOSITION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(onRoad ? v->getPositionOnLane()
                                 : INVALID_DOUBLE_VALUE);
      break;
    case VAR_CO2EMISSION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? v->getCO2Emissions()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_COEMISSION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
    ...
    case VAR_PMXEMISSION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? v->getPMxEmissions()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_NOXEMISSION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? v->getNOxEmissions()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_FUELCONSUMPTION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? v->getFuelConsumption()
                                  : INVALID_DOUBLE_VALUE);
      break;
    case VAR_NOISEEMISSION:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(visible ? v->getHarmonoise_NoiseEmissions()
                                  : INVALID_DOUBLE_VALUE);
      break;
    ...
    case VAR_LEADER: {
      ...
      if (!server.readTypeCheckingDouble(inputStorage, dist)) {
        return server.writeErrorStatusCmd(CMD_GET_VEHICLE_VARIABLE,
                                          "Leader retrieval requires a double.",
                                          outputStorage);
      }
      std::pair<const MSVehicle *const, SUMOReal> leaderInfo =
          v->getLeader(dist);
    ...
    case VAR_EDGE_TRAVELTIME: {
      ...
      if (inputStorage.readInt() != 2) {
        return server.writeErrorStatusCmd(
            CMD_GET_VEHICLE_VARIABLE,
            "Retrieval of travel time requires time, and edge as parameter.",
            outputStorage);
      }
      // time
      int time = 0;
      if (!server.readTypeCheckingInt(inputStorage, time)) {
        return server.writeErrorStatusCmd(
            CMD_GET_VEHICLE_VARIABLE, "Retrieval of travel time requires the "
                                      "referenced time as first parameter.",
            outputStorage);
      }
      // edge
      std::string edgeID;
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(
            CMD_GET_VEHICLE_VARIABLE, "Retrieval of travel time requires the "
                                      "referenced edge as second parameter.",
            outputStorage);
      }
      ...
      if (edge == 0) {
        return server.writeErrorStatusCmd(
            CMD_GET_VEHICLE_VARIABLE,
            "Referenced edge '" + edgeID + "' is not known.", outputStorage);
      }
      // retrieve
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      SUMOReal value;
      if (!v->getWeightsStorage().retrieveExistingTravelTime(edge, time,
                                                             value)) {
    ...
    case VAR_EDGE_EFFORT: {
      ...
      if (!server.readTypeCheckingInt(inputStorage, time)) {
        return server.writeErrorStatusCmd(CMD_GET_VEHICLE_VARIABLE,
                                          "Retrieval of effort requires the "
                                          "referenced time as first parameter.",
                                          outputStorage);
      }
      // edge
      ...
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(
            CMD_GET_VEHICLE_VARIABLE, "Retrieval of effort requires the "
                                      "referenced edge as second parameter.",
            outputStorage);
      }
      MSEdge *edge = MSEdge::dictionary(edgeID);
    ...
    case VAR_BEST_LANES: {
      ...
      const std::vector<MSVehicle::LaneQ> &bestLanes =
          onRoad ? v->getBestLanes() : std::vector<MSVehicle::LaneQ>();
      tempContent.writeInt((int)bestLanes.size());
      ++cnt;
      for (std::vector<MSVehicle::LaneQ>::const_iterator i = bestLanes.begin();
           i != bestLanes.end(); ++i) {
        ...
        lq.allowsContinuation ? tempContent.writeUnsignedByte(1)
                              : tempContent.writeUnsignedByte(0);
        ++cnt;
        std::vector<std::string> bestContIDs;
        for (std::vector<MSLane *>::const_iterator j =
                 lq.bestContinuations.begin();
             j != lq.bestContinuations.end(); ++j) {
          if ((*j) != 0) {
    ...
    case VAR_STOPSTATE: {
      ...
      if (v->isStopped()) {
        ...
        b = 1 + (stop.parking ? 2 : 0) + (stop.triggered ? 4 : 0) +
            (stop.containerTriggered ? 8 : 0) + (stop.busstop != 0 ? 16 : 0) +
    ...
    case VAR_DISTANCE: {
      ...
      SUMOReal distance =
          onRoad
              ? v->getRoute().getDistanceBetween(
                    v->getDepartPos(), v->getPositionOnLane(),
                    v->getRoute().getEdges()[0], &v->getLane()->getEdge())
              : INVALID_DOUBLE_VALUE;
    ...
    case VAR_ALLOWED_SPEED:
      tempMsg.writeUnsignedByte(TYPE_DOUBLE);
      tempMsg.writeDouble(onRoad ? v->getLane()->getVehicleMaxSpeed(v)
                                 : INVALID_DOUBLE_VALUE);
      break;
    ...
    case VAR_GET_SPEED_AND_ACCELERATION: {
      ...
      model->getVehicleInformation((const MSVehicle *)v, speed, acceleration,
                                   controllerAcceleration, position, time);
    ...
    case VAR_GET_DISTANCE_TO_END:
      ...
      distanceToEnd = route->getDistanceBetween(
          positionOnEdge, lastEdge->getLanes()[0]->getLength(), currentEdge,
          lastEdge);
    ...
    case VAR_GET_DISTANCE_FROM_BEGIN: {
      ...
      positionOnEdge = v->getPositionOnLane();
      distanceFromBegin =
          route->getDistanceBetween(0, positionOnEdge, firstEdge, currentEdge);
    ...
    case VAR_GET_GENERIC_INFORMATION: {
      ...
      const MSCFModel_CC *model;
      model = dynamic_cast<const MSCFModel_CC *>(
          &v->getVehicleType().getCarFollowModel());
      assert(model);
      ...
      inputStorage.readBuffer((unsigned char *)&request,
                              sizeof(struct Plexe::CCDataHeader));
      ...
      size = model->getGenericInformation((const MSVehicle *)v, request,
                                          reqParams, content);
      // write response header
      response.type = request.type;
      response.size = size;
      tempMsg.writeBuffer((unsigned char *)&response,
                          sizeof(struct Plexe::CCDataHeader));
    ...
    default:
      TraCIServerAPI_VehicleType::getVariable(variable, v->getVehicleType(),
                                              tempMsg);
      break;
    }
  }
...
bool TraCIServerAPI_Vehicle::processSet(TraCIServer &server,
  ...
  if (variable != CMD_STOP && variable != CMD_CHANGELANE &&
      ...
      variable != CMD_RESUME && variable != VAR_TYPE &&
      ...
      variable != VAR_ACCEL && variable != VAR_DECEL &&
      variable != VAR_IMPERFECTION && variable != VAR_TAU &&
      variable != VAR_LANECHANGE_MODE && variable != VAR_SPEED &&
      variable != VAR_SPEEDSETMODE && variable != VAR_COLOR &&
      variable != ADD && variable != ADD_FULL && variable != REMOVE &&
      variable != VAR_MOVE_TO_VTD &&
      variable != VAR_PARAMETER /* && variable != VAR_SPEED_TIME_LINE &&
                                   variable != VAR_LANE_TIME_LINE*/
      ...
      variable != VAR_SET_FIXED_LANE && variable != VAR_SET_ACC_HEADWAY_TIME &&
      variable != VAR_SET_FIXED_ACCELERATION &&
      variable != VAR_SET_CACC_SPACING &&
      variable != VAR_SET_GENERIC_INFORMATION) {
    return server.writeErrorStatusCmd(
        CMD_SET_VEHICLE_VARIABLE,
        "Change Vehicle State: unsupported variable " + toHex(variable, 2) +
            " specified",
        outputStorage);
  }
  // id
  ...
  case CMD_STOP: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Stop needs a compound object description.",
          outputStorage);
    }
    ...
    if (compoundSize < 4 || compoundSize > 7) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Stop needs a compound object description of four to seven items.",
          outputStorage);
    }
    ...
    if (!server.readTypeCheckingDouble(inputStorage, pos)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The second stop parameter must be the "
                                        "end position along the edge given as "
                                        "a double.",
                                        outputStorage);
    }
    int laneIndex = 0;
    ...
    if (!server.readTypeCheckingInt(inputStorage, waitTime)) {
      return server.writeErrorStatusCmd(CMD_GET_VEHICLE_VARIABLE,
                                        "The fourth stop parameter must be the "
                                        "waiting time given as an integer.",
                                        outputStorage);
    }
    // optional stop flags
    bool parking = false;
    ...
    if (compoundSize >= 5) {
      ...
      if (!server.readTypeCheckingByte(inputStorage, stopFlags)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "The fifth stop parameter must be a "
                                          "byte indicating its "
                                          "parking/triggered status.",
                                          outputStorage);
      }
    ...
    if (compoundSize >= 6) {
      ...
      if (!server.readTypeCheckingDouble(inputStorage, tmp)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "The sixth stop parameter must be "
                                          "the start position along the edge "
                                          "given as a double.",
                                          outputStorage);
      }
    ...
    if (compoundSize >= 7) {
      if (!server.readTypeCheckingInt(inputStorage, until)) {
        return server.writeErrorStatusCmd(
            CMD_SET_VEHICLE_VARIABLE, "The seventh stop parameter must be the "
                                      "waiting end time given as integer.",
            outputStorage);
      }
    }
    std::string error;
    if (isBusStop || isContainerStop) {
      ...
      if (!v->addTraciBusOrContainerStop(roadId, waitTime, until, parking,
                                         triggered, containerTriggered,
                                         isContainerStop, error)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                          outputStorage);
      }
    } else {
      // check
      if (startPos < 0) {
        return server.writeErrorStatusCmd(
            CMD_SET_VEHICLE_VARIABLE, "Position on lane must not be negative.",
            outputStorage);
      }
      if (pos < startPos) {
        return server.writeErrorStatusCmd(
      ...
      if (road == 0) {
        return server.writeErrorStatusCmd(
            CMD_SET_VEHICLE_VARIABLE, "Unable to retrieve road with given id.",
            outputStorage);
      }
      ...
      // Forward command to vehicle
      if (!v->addTraciStop(allLanes[laneIndex], startPos, pos, waitTime, until,
                           parking, triggered, containerTriggered, error)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                          outputStorage);
      }
    }
  ...
  case CMD_RESUME: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      server.writeStatusCmd(CMD_SET_VEHICLE_VARIABLE, RTYPE_ERR,
                            "Resuming requires a compound object.",
                            outputStorage);
      return false;
    }
    if (inputStorage.readInt() != 0) {
      server.writeStatusCmd(CMD_SET_VEHICLE_VARIABLE, RTYPE_ERR,
                            "Resuming should obtain an empty compound object.",
                            outputStorage);
      return false;
    }
    if (!static_cast<MSVehicle *>(v)->hasStops()) {
      server.writeStatusCmd(CMD_SET_VEHICLE_VARIABLE, RTYPE_ERR,
                            "Failed to resume vehicle '" + v->getID() +
                                "', it has no stops.",
                            outputStorage);
      return false;
    }
    if (!static_cast<MSVehicle *>(v)->resumeFromStopping()) {
      ...
      server.writeStatusCmd(CMD_SET_VEHICLE_VARIABLE, RTYPE_ERR,
                            "Failed to resume a non parking vehicle '" +
                                v->getID() + "', " + posStr,
                            outputStorage);
      return false;
    }
  ...
  case CMD_CHANGELANE: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Lane change needs a compound object description.", outputStorage);
    }
    if (inputStorage.readInt() != 2) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Lane change needs a compound object description of two items.",
          outputStorage);
    }
    // Lane ID
    int laneIndex = 0;
    if (!server.readTypeCheckingByte(inputStorage, laneIndex)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The first lane change parameter must "
                                        "be the lane index given as a byte.",
                                        outputStorage);
    }
    // stickyTime
    int stickyTime = 0;
    if (!server.readTypeCheckingInt(inputStorage, stickyTime)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The second lane change parameter must "
                                        "be the duration given as an integer.",
                                        outputStorage);
    }
    if ((laneIndex < 0) ||
        (laneIndex >= (int)(v->getEdge()->getLanes().size()))) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "No lane with index '" + toString(laneIndex) + "' on road '" +
              v->getEdge()->getID() + "'.",
          outputStorage);
    }
    // Forward command to vehicle
    ...
    // clang-format on
    laneTimeLine.push_back(
        std::make_pair(MSNet::getInstance()->getCurrentTimeStep(), laneIndex));
    laneTimeLine.push_back(std::make_pair(
  ...
  case VAR_LANE_TIME_LINE: {
      if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
          return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, "Lane
  change needs a compound object description.", outputStorage);
      }
      if (inputStorage.readInt() != 2) {
          return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, "Lane
  change needs a compound object description of two items.", outputStorage);
      }
      // Lane ID
      int laneIndex = 0;
      if (!server.readTypeCheckingByte(inputStorage, laneIndex)) {
          return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, "The first
  lane change parameter must be the lane index given as a byte.",
  outputStorage);
      }
      // stickyTime
      SUMOTime stickyTime = 0;
      if (!server.readTypeCheckingInt(inputStorage, stickyTime)) {
          return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, "The
  second lane change parameter must be the duration given as an integer.",
  outputStorage);
      }
      if ((laneIndex < 0) || (laneIndex >=
  (int)(v->getEdge()->getLanes().size()))) {
          return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, "No lane
  existing with given id on the current road", outputStorage);
      }
      // Forward command to vehicle
      std::vector<std::pair<SUMOTime, unsigned int> > laneTimeLine;
      laneTimeLine.push_back(std::make_pair(MSNet::getInstance()->getCurrentTimeStep(),
  laneIndex));
      laneTimeLine.push_back(std::make_pair(MSNet::getInstance()->getCurrentTimeStep()
  + stickyTime, laneIndex));
      v->getInfluencer().setLaneTimeLine(laneTimeLine);
      MSVehicle::ChangeRequest req =
  v->getInfluencer().checkForLaneChanges(MSNet::getInstance()->getCurrentTimeStep(),
  ...
  */
  case CMD_SLOWDOWN: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Slow down needs a compound object description.", outputStorage);
    }
    if (inputStorage.readInt() != 2) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
    ...
    if (!server.readTypeCheckingInt(inputStorage, duration)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The second slow down parameter must "
                                        "be the duration given as an integer.",
                                        outputStorage);
    }
    if (duration < 0 ||
        STEPS2TIME(MSNet::getInstance()->getCurrentTimeStep()) +
                STEPS2TIME(duration) >
            STEPS2TIME(SUMOTime_MAX - DELTA_T)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "Invalid time interval", outputStorage);
    }
    // clang-format off
    std::vector<std::pair<SUMOTime, SUMOReal> > speedTimeLine;
    // clang-format on
    speedTimeLine.push_back(std::make_pair(
        MSNet::getInstance()->getCurrentTimeStep(), v->getSpeed()));
    speedTimeLine.push_back(std::make_pair(
  ...
  case CMD_CHANGETARGET: {
    ...
    if (!server.readTypeCheckingString(inputStorage, edgeID)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "Change target requires a string "
                                        "containing the id of the new "
                                        "destination edge as parameter.",
                                        outputStorage);
    }
    const MSEdge *destEdge = MSEdge::dictionary(edgeID);
    if (destEdge == 0) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Can not retrieve road with ID " + edgeID,
          outputStorage);
    }
    ...
    MSNet::getInstance()->getRouterTT().compute(
        currentEdge, destEdge, (const MSVehicle *const)v,
    ...
    if (!v->replaceRouteEdges(newRoute, v->getLane() == 0)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Route replacement failed for " + v->getID(), outputStorage);
    }
  } break;
  case VAR_TYPE: {
    std::string vTypeID;
    if (!server.readTypeCheckingString(inputStorage, vTypeID)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "The vehicle type id must be given as a string.", outputStorage);
    }
    MSVehicleType *vehicleType =
        MSNet::getInstance()->getVehicleControl().getVType(vTypeID);
    if (vehicleType == 0) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "The vehicle type '" + vTypeID + "' is not known.", outputStorage);
    }
  ...
  case VAR_ROUTE_ID: {
    ...
    if (!server.readTypeCheckingString(inputStorage, rid)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "The route id must be given as a string.",
          outputStorage);
    }
    const MSRoute *r = MSRoute::dictionary(rid);
    if (r == 0) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The route '" + rid + "' is not known.",
                                        outputStorage);
    }
  ...
  case VAR_ROUTE: {
    ...
    if (!server.readTypeCheckingStringList(inputStorage, edgeIDs)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "A route must be defined as a list of edge ids.", outputStorage);
    }
  ...
  case VAR_EDGE_TRAVELTIME: {
    ...
    if (parameterCount == 4) {
      ...
      if (!server.readTypeCheckingInt(inputStorage, begTime)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting travel time using 4 "
                                          "parameters requires the begin time "
                                          "as first parameter.",
                                          outputStorage);
      }
      // begin time
      if (!server.readTypeCheckingInt(inputStorage, endTime)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting travel time using 4 "
                                          "parameters requires the end time as "
                                          "second parameter.",
                                          outputStorage);
      }
      // edge
      ...
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting travel time using 4 "
                                          "parameters requires the referenced "
                                          "edge as third parameter.",
                                          outputStorage);
      }
      MSEdge *edge = MSEdge::dictionary(edgeID);
      if (edge == 0) {
      ...
      if (!server.readTypeCheckingDouble(inputStorage, value)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting travel time using 4 "
                                          "parameters requires the travel time "
                                          "as double as fourth parameter.",
                                          outputStorage);
      }
      // retrieve
      ...
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting travel time using 2 "
                                          "parameters requires the referenced "
                                          "edge as first parameter.",
                                          outputStorage);
      }
      MSEdge *edge = MSEdge::dictionary(edgeID);
      if (edge == 0) {
      ...
      v->getWeightsStorage().addTravelTime(edge, SUMOReal(0),
                                           SUMOReal(SUMOTime_MAX), value);
    } else if (parameterCount == 1) {
      // edge
      std::string edgeID;
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting travel time using 1 "
                                          "parameter requires the referenced "
                                          "edge as first parameter.",
                                          outputStorage);
      }
      MSEdge *edge = MSEdge::dictionary(edgeID);
      ...
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting travel time requires 1, 2, or 4 parameters.", outputStorage);
    }
  } break;
  case VAR_EDGE_EFFORT: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting effort requires a compound object.", outputStorage);
    }
    int parameterCount = inputStorage.readInt();
    if (parameterCount == 4) {
      // begin time
      ...
      if (!server.readTypeCheckingInt(inputStorage, begTime)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 4 parameters "
                                          "requires the begin time as first "
                                          "parameter.",
                                          outputStorage);
      }
      // begin time
      if (!server.readTypeCheckingInt(inputStorage, endTime)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 4 parameters "
                                          "requires the end time as second "
                                          "parameter.",
                                          outputStorage);
      }
      // edge
      ...
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 4 parameters "
                                          "requires the referenced edge as "
                                          "third parameter.",
                                          outputStorage);
      }
      MSEdge *edge = MSEdge::dictionary(edgeID);
      ...
      if (!server.readTypeCheckingDouble(inputStorage, value)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 4 parameters "
                                          "requires the travel time as fourth "
                                          "parameter.",
                                          outputStorage);
      }
      // retrieve
      ...
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 2 parameters "
                                          "requires the referenced edge as "
                                          "first parameter.",
                                          outputStorage);
      }
      ...
      if (!server.readTypeCheckingDouble(inputStorage, value)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 2 parameters "
                                          "requires the travel time as second "
                                          "parameter.",
                                          outputStorage);
      }
      // retrieve
      ...
      v->getWeightsStorage().addEffort(edge, SUMOReal(0),
                                       SUMOReal(SUMOTime_MAX), value);
    } else if (parameterCount == 1) {
      // edge
      std::string edgeID;
      if (!server.readTypeCheckingString(inputStorage, edgeID)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                          "Setting effort using 1 parameter "
                                          "requires the referenced edge as "
                                          "first parameter.",
                                          outputStorage);
      }
      ...
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting effort requires 1, 2, or 4 parameters.", outputStorage);
    }
  } break;
  case CMD_REROUTE_TRAVELTIME: {
    ...
    if (inputStorage.readInt() != 0) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Rerouting should obtain an empty compound object.", outputStorage);
    }
    v->reroute(MSNet::getInstance()->getCurrentTimeStep(),
  ...
  case CMD_REROUTE_EFFORT: {
    ...
    v->reroute(MSNet::getInstance()->getCurrentTimeStep(),
               MSNet::getInstance()->getRouterEffort());
  } break;
  case VAR_SIGNALS: {
    int signals = 0;
  ...
  case VAR_MOVE_TO: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting position requires a compound object.", outputStorage);
    }
    if (inputStorage.readInt() != 2) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting position should obtain the lane id and the position.",
          outputStorage);
    }
    // lane ID
    std::string laneID;
    if (!server.readTypeCheckingString(inputStorage, laneID)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The first parameter for setting a "
                                        "position must be the lane ID given as "
                                        "a string.",
                                        outputStorage);
    }
    // position on lane
    ...
    if (!server.readTypeCheckingDouble(inputStorage, position)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The second parameter for setting a "
                                        "position must be the position given "
                                        "as a double.",
                                        outputStorage);
    }
    // process
    ...
    if (!v->willPass(&destinationEdge)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Vehicle '" + laneID + "' may be set onto an edge to pass only.",
          outputStorage);
    }
  ...
  case VAR_SPEED: {
    ...
    if (!server.readTypeCheckingDouble(inputStorage, speed)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "Setting speed requires a double.",
                                        outputStorage);
    }
    // clang-format off
    std::vector<std::pair<SUMOTime, SUMOReal> > speedTimeLine;
    ...
    if (speed >= 0) {
      speedTimeLine.push_back(
          std::make_pair(MSNet::getInstance()->getCurrentTimeStep(), speed));
  ...
  case VAR_SPEEDSETMODE: {
    ...
    if (!server.readTypeCheckingInt(inputStorage, speedMode)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Setting speed mode requires an integer.",
          outputStorage);
    }
  ...
  case VAR_LANECHANGE_MODE: {
    ...
    if (!server.readTypeCheckingInt(inputStorage, laneChangeMode)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting lane change mode requires an integer.", outputStorage);
    }
  ...
  case ADD: {
    if (v != 0) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "The vehicle " + id + " to add already exists.", outputStorage);
    }
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Adding a vehicle requires a compound object.", outputStorage);
    }
    if (inputStorage.readInt() != 6) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Adding a vehicle needs six parameters.",
          outputStorage);
    }
    ...
    if (!server.readTypeCheckingString(inputStorage, vTypeID)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "First parameter (type) requires a string.",
          outputStorage);
    }
    MSVehicleType *vehicleType =
        MSNet::getInstance()->getVehicleControl().getVType(vTypeID);
    ...
    if (!server.readTypeCheckingString(inputStorage, routeID)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Second parameter (route) requires a string.", outputStorage);
    }
    ...
    if (!server.readTypeCheckingInt(inputStorage, depart)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Third parameter (depart) requires an integer.", outputStorage);
    }
    if (depart < 0) {
      const int proc = -depart;
      if (proc >= static_cast<int>(DEPART_DEF_MAX)) {
        return server.writeErrorStatusCmd(
            CMD_SET_VEHICLE_VARIABLE, "Invalid departure time.", outputStorage);
      }
      ...
      WRITE_WARNING("Departure time for vehicle '" + id +
    ...
    if (!server.readTypeCheckingDouble(inputStorage, pos)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Fourth parameter (position) requires a double.", outputStorage);
    }
    vehicleParams.departPos = pos;
    if (vehicleParams.departPos < 0) {
      const int proc = static_cast<int>(-vehicleParams.departPos);
      if (fabs(proc + vehicleParams.departPos) > NUMERICAL_EPS ||
          proc >= static_cast<int>(DEPART_POS_DEF_MAX) ||
          proc == static_cast<int>(DEPART_POS_GIVEN)) {
        return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
    ...
    if (!server.readTypeCheckingDouble(inputStorage, speed)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Fifth parameter (speed) requires a double.", outputStorage);
    }
    ...
    if (!server.readTypeCheckingByte(inputStorage, vehicleParams.departLane)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Sixth parameter (lane) requires a byte.",
          outputStorage);
    }
    ...
    if (vehicleParams.departLane < 0) {
      ...
      if (proc >= static_cast<int>(DEPART_LANE_DEF_MAX)) {
        return server.writeErrorStatusCmd(
            CMD_SET_VEHICLE_VARIABLE, "Invalid departure lane.", outputStorage);
      }
    ...
    try {
      SUMOVehicle *vehicle =
          MSNet::getInstance()->getVehicleControl().buildVehicle(
              params, route, vehicleType, true, false);
      MSNet::getInstance()->getVehicleControl().addVehicle(vehicleParams.id,
                                                           vehicle);
      MSNet::getInstance()->getInsertionControl().add(vehicle);
    } catch (ProcessError &e) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, e.what(),
                                        outputStorage);
    }
  } break;
  case ADD_FULL: {
    ...
    if (!server.readTypeCheckingString(inputStorage, helper)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Third parameter (depart) requires an string.", outputStorage);
    }
    if (!SUMOVehicleParameter::parseDepart(
            helper, "vehicle", id, vehicleParams.depart,
            vehicleParams.departProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    if (vehicleParams.departProcedure == DEPART_GIVEN &&
        vehicleParams.depart < MSNet::getInstance()->getCurrentTimeStep()) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
    ...
    if (!server.readTypeCheckingString(inputStorage, helper)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Fourth parameter (depart lane) requires a string.", outputStorage);
    }
    if (!SUMOVehicleParameter::parseDepartLane(
            helper, "vehicle", id, vehicleParams.departLane,
            vehicleParams.departLaneProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    if (!server.readTypeCheckingString(inputStorage, helper)) {
    ...
    if (!SUMOVehicleParameter::parseDepartPos(
            helper, "vehicle", id, vehicleParams.departPos,
            vehicleParams.departPosProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    if (!server.readTypeCheckingString(inputStorage, helper)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Sixth parameter (depart speed) requires a string.", outputStorage);
    }
    if (!SUMOVehicleParameter::parseDepartSpeed(
            helper, "vehicle", id, vehicleParams.departSpeed,
            vehicleParams.departSpeedProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    ...
    if (!server.readTypeCheckingString(inputStorage, helper)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Seventh parameter (arrival lane) requires a string.", outputStorage);
    }
    if (!SUMOVehicleParameter::parseArrivalLane(
            helper, "vehicle", id, vehicleParams.arrivalLane,
            vehicleParams.arrivalLaneProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    if (!server.readTypeCheckingString(inputStorage, helper)) {
    ...
    if (!SUMOVehicleParameter::parseArrivalPos(
            helper, "vehicle", id, vehicleParams.arrivalPos,
            vehicleParams.arrivalPosProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    if (!server.readTypeCheckingString(inputStorage, helper)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Ninth parameter (arrival speed) requires a string.", outputStorage);
    }
    if (!SUMOVehicleParameter::parseArrivalSpeed(
            helper, "vehicle", id, vehicleParams.arrivalSpeed,
            vehicleParams.arrivalSpeedProcedure, error)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE, error,
                                        outputStorage);
    }
    ...
    if (!server.readTypeCheckingString(inputStorage, vehicleParams.fromTaz)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Tenth parameter (from taz) requires a string.", outputStorage);
    }
    if (!server.readTypeCheckingString(inputStorage, vehicleParams.toTaz)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Eleventh parameter (to taz) requires a string.", outputStorage);
    }
    if (!server.readTypeCheckingString(inputStorage, vehicleParams.line)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Twelth parameter (line) requires a string.", outputStorage);
    }
    ...
    if (!server.readTypeCheckingInt(inputStorage, num)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "13th parameter (person capacity) requires an int.", outputStorage);
    }
    if (!server.readTypeCheckingInt(inputStorage, num)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "14th parameter (person number) requires an int.", outputStorage);
    }
  ...
  case REMOVE: {
    ...
    default:
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Unknown removal status.", outputStorage);
    }
    if (v->hasDeparted()) {
  ...
  case VAR_MOVE_TO_VTD: {
    if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting VTD vehicle requires a compound object.", outputStorage);
    }
    if (inputStorage.readInt() != 5) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE,
          "Setting VTD vehicle should obtain: edgeID, lane, x, y, angle.",
          outputStorage);
    }
    // edge ID
    std::string edgeID;
    if (!server.readTypeCheckingString(inputStorage, edgeID)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The first parameter for setting a VTD "
                                        "vehicle must be the edge ID given as "
                                        "a string.",
                                        outputStorage);
    }
    // lane index
    ...
    if (!server.readTypeCheckingInt(inputStorage, laneNum)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "The second parameter for setting a VTD "
                                    "vehicle must be lane given as an int.",
          outputStorage);
    }
    // x
    double x = 0;
    ...
    if (!server.readTypeCheckingDouble(inputStorage, x)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The third parameter for setting a VTD "
                                        "vehicle must be the x-position given "
                                        "as a double.",
                                        outputStorage);
    }
    // y
    if (!server.readTypeCheckingDouble(inputStorage, y)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The fourth parameter for setting a "
                                        "VTD vehicle must be the y-position "
                                        "given as a double.",
                                        outputStorage);
    }
    // angle
    if (!server.readTypeCheckingDouble(inputStorage, origAngle)) {
      return server.writeErrorStatusCmd(CMD_SET_VEHICLE_VARIABLE,
                                        "The fifth parameter for setting a VTD "
                                        "vehicle must be the angle given as a "
                                        "double.",
                                        outputStorage);
    }
    // process
...
#ifdef DEBUG_VTD
    std::cout << std::endl
              << "begin vehicle " << v->getID() << " vehPos:" << vehPos
              << " lane:" << v->getLane()->getID() << std::endl;
    std::cout << " want pos:" << pos << " edge:" << edgeID
              << " laneNum:" << laneNum << " origAngle:" << origAngle
    ...
    int routeOffset = 0;
    /* EGO vehicle is known to have a fixed route. @todo make this into a
     * parameter of the TraCI call */
    if (v->getID() != "VTD_EGO") {
      ...
      bool found = vtdMap_matchingRoutePosition(
          pos, origID, *v, bestDistance, &lane, lanePos, routeOffset, edges);
      SUMOReal maxRouteDistance = 100;
      // use the best we have
      if (found && maxRouteDistance > bestDistance) {
        server.setVTDControlled(v, lane, lanePos, angle, routeOffset, edges,
                                MSNet::getInstance()->getCurrentTimeStep());
      }
    } else {
      // case b): vehicle does not follow a pre-fixed route (regard the limiting
      // factor in maxRouteDistance)
      bool found = vtdMap(pos, origID, angle, *v, server, bestDistance, &lane,
                          lanePos, routeOffset, edges);
      SUMOReal maxRouteDistance = 100;
      // use the best we have
      if (found && maxRouteDistance > bestDistance) {
        server.setVTDControlled(v, lane, lanePos, angle, routeOffset, edges,
  ...
  case VAR_SPEED_FACTOR: {
    ...
    if (!server.readTypeCheckingDouble(inputStorage, factor)) {
      return server.writeErrorStatusCmd(
          CMD_SET_VEHICLE_VARIABLE, "Setting speed factor requires a double.",
          outputStorage);
    }
  ...
  case VAR_SET_ACTIVE_CONTROLLER: {
    ...
    model->setActiveController((const MSVehicle *)v,
  ...
  case VAR_SET_LANE_CHANGE_ACTION: {
    ...
    assert(action >= Plexe::DRIVER_CHOICE &&
           action <= Plexe::STAY_IN_CURRENT_LANE);

    model->setLaneChangeAction(
        (MSVehicle *)v, (enum Plexe::PLATOONING_LANE_CHANGE_ACTION)action);
  ...
  case VAR_SET_CONTROLLER_FAKE_DATA: {
    ...
    // TODO: only basic idea for data needed for joining. might need to be
    // extended
    /* we expect to get from the upper application the data that the CACC needs,
     * i.e.:
     * - front distance, front speed and front vehicle acceleration: this
     * information
     *   regards the car that the vehicle joining the platoon will have directly
     * in
     *   front. this data might be real or might be fake: for example, if the
     * platoon
     *   management algorithm decides to set the vehicle as the new leader,
     * there won't
     *   be a car in front, and the fake data will be used only for positioning.
     * in the
     *   case of fake data, acceleration must be set to 0
     * - leader front speed and acceleration: this information is the same as
     * previously
     *   described for vehicle in front, but regards the leader. again, if the
     * vehicle
    ...
    model->setControllerFakeData(v, frontDistance, frontSpeed,
                                 frontAcceleration, leaderSpeed,
                                 leaderAcceleration);
  ...
  case VAR_SET_GENERIC_INFORMATION: {
    ...
    // copy the header
    inputStorage.readBuffer((unsigned char *)&header,
                            sizeof(struct Plexe::CCDataHeader));
  ...
  default:
    try {
      if (!TraCIServerAPI_VehicleType::setVariable(
              CMD_SET_VEHICLE_VARIABLE, variable, getSingularType(v), server,
  ...
  server.writeStatusCmd(CMD_SET_VEHICLE_VARIABLE, RTYPE_OK, warning,
                        outputStorage);
  return true;
}

bool TraCIServerAPI_Vehicle::processSetGet(TraCIServer &server,
                                           tcpip::Storage &inputStorage,
                                           tcpip::Storage &outputStorage) {
  std::string warning = ""; // additional description for response
  // variable
  int variable = inputStorage.readUnsignedByte();
  if (variable != VAR_SET_GET_CONTRACT_SIGNATURE) {
    return server.writeErrorStatusCmd(
        CMD_SET_GET_VEHICLE_VARIABLE,
        "Set and Get Vehicle State: unsupported variable " +
            toHex(variable, 2) + " specified",
        outputStorage);
  }
  // id
  std::string id = inputStorage.readString();
  ...
  if (sumoVehicle == 0) {
    if (shouldExist) {
      return server.writeErrorStatusCmd(CMD_SET_GET_VEHICLE_VARIABLE,
                                        "Vehicle '" + id + "' is not known",
                                        outputStorage);
    }
  }
  ...
  // build output buffer
  tcpip::Storage tempMsg;
  tempMsg.writeUnsignedByte(RESPONSE_SET_GET_VEHICLE_VARIABLE);
  tempMsg.writeUnsignedByte(variable);
  ...
  case VAR_SET_GET_CONTRACT_SIGNATURE: {
    ...
    //  ((unsigned char *)signatures)[i] = inputStorage.readChar();
    //}
    ...
    //}
    v->myEnclave->newContractChainGetSignature(
        contract, &return_signature, num_signatures, signatures, &compute_time);
    tempMsg.writeBuffer((unsigned char *)&return_signature,
                        sizeof(cp_ec256_signature_t));
    tempMsg.writeDouble(compute_time);
  } break;
  }

  // respond to client
  server.writeStatusCmd(CMD_SET_GET_VEHICLE_VARIABLE, RTYPE_OK, "",
                        outputStorage);
  server.writeResponseWithLength(outputStorage, tempMsg);
  return true;
}

bool TraCIServerAPI_Vehicle::vtdMap(
    const Position &pos, const std::string &origID, const SUMOReal angle,
    MSVehicle &v, TraCIServer &server, SUMOReal &bestDistance, MSLane **lane,
    SUMOReal &lanePos, int &routeOffset, ConstMSEdgeVector &edges) {
  ...
  for (std::set<std::string>::const_iterator j = into.begin(); j != into.end();
       ++j) {
    ...
    bool onRoute = false;
    // the next if/the clause sets "onRoute", "prevEdge", and "nextEdge",
    // depending on
    ...
    if (ef != MSEdge::EDGEFUNCTION_INTERNAL) {
      ...
      if (v.getLane()->getEdge().getPurpose() ==
          MSEdge::EDGEFUNCTION_INTERNAL) {
        ++routePosition;
      }
      ConstMSEdgeVector::const_iterator edgePos =
          std::find(ev.begin() + routePosition, ev.end(), e);
      ...
      if (edgePos == ev.end() - 1 && v.getEdge() == e) {
        ...
        onRoute &= v.getEdge()->getLanes()[0]->getLength() >
                   v.getPositionOnLane() + SPEED2DIST(speed);
      }
      // save prior and next edges
      prevEdge = e;
      nextEdge = !onRoute || edgePos == ev.end() - 1 ? 0 : *(edgePos + 1);
#ifdef DEBUG_VTD_ANGLE
      std::cout << "normal:" << e->getID() << " prev:" << prevEdge->getID()
                << " next:";
      if (nextEdge != 0) {
      ...
      prevEdge = e;
      while (prevEdge != 0 &&
             prevEdge->getPurpose() == MSEdge::EDGEFUNCTION_INTERNAL) {
      ...
      const ConstMSEdgeVector &ev = v.getRoute().getEdges();
      ConstMSEdgeVector::const_iterator prevEdgePos =
          std::find(ev.begin() + v.getRoutePosition(), ev.end(), prevEdge);
      nextEdge = e;
      while (nextEdge != 0 &&
             nextEdge->getPurpose() == MSEdge::EDGEFUNCTION_INTERNAL) {
        nextEdge = nextEdge->getSuccessors()[0]; // should be only one for an
                                                 // internal edge
      }
...
#ifdef DEBUG_VTD_ANGLE
      std::cout << "internal:" << e->getID() << " prev:" << prevEdge->getID()
    ...
    for (std::vector<MSLane *>::const_iterator k = lanes.begin();
         k != lanes.end(); ++k) {
      MSLane *lane = *k;
      ...
      bool sameEdge = &lane->getEdge() == &v.getLane()->getEdge() &&
                      v.getEdge()->getLanes()[0]->getLength() >
                          v.getPositionOnLane() + SPEED2DIST(speed);
/*
...
*/
...
#endif
      lane2utility[lane] =
          LaneUtility(dist, GeomHelper::getMinAngleDiff(angle, langle),
                      lane->getParameter("origId", "") == origID, onRoute,
                      sameEdge, prevEdge, nextEdge);
  ...
  MSLane *bestLane = 0;
  for (std::map<MSLane *, LaneUtility>::iterator i = lane2utility.begin();
       i != lane2utility.end(); ++i) {
    MSLane *l = (*i).first;
    ...
    SUMOReal sameEdgeN =
        u.sameEdge ? MIN2(v.getEdge()->getLength() / speed, (SUMOReal)1.) : 0;
    SUMOReal value = (distN * .35 + angleDiffN * 0.35 /*.5 */
                      + idN * .1 + onRouteN * 0.1 + sameEdgeN * 0.1);
#ifdef DEBUG_VTD
    std::cout << " x; l:" << l->getID() << " d:" << u.dist << " dN:" << distN
              << " aD:" << angleDiffN << " ID:" << idN << " oRN:" << onRouteN
              << " sEN:" << sameEdgeN << " value:" << value << std::endl;
#endif
  ...
  *lane = bestLane;
  ...
  if (u.onRoute) {
    const ConstMSEdgeVector &ev = v.getRoute().getEdges();
    ConstMSEdgeVector::const_iterator prevEdgePos =
        std::find(ev.begin(), ev.end(), prevEdge);
    routeOffset = (int)std::distance(ev.begin(), prevEdgePos);
    // std::cout << SIMTIME << "vtdMap vehicle=" << v.getID() << " currLane="
    // <<
    // v.getLane()->getID() << " routeOffset=" << routeOffset << " edges=" <<
    // toString(ev) << " bestLane=" << bestLane->getID() << " prevEdge=" <<
    // prevEdge->getID() << "\n";
  } else {
...
bool TraCIServerAPI_Vehicle::findCloserLane(const MSEdge *edge,
  ...
  bool newBest = false;
  for (std::vector<MSLane *>::const_iterator k = lanes.begin();
       k != lanes.end() && bestDistance > POSITION_EPS; ++k) {
    MSLane *candidateLane = *k;
    const SUMOReal dist =
        candidateLane->getShape().distance(pos); // get distance
#ifdef DEBUG_VTD
    std::cout << "   b at lane " << candidateLane->getID() << " dist:" << dist
...
bool TraCIServerAPI_Vehicle::vtdMap_matchingRoutePosition(
    const Position &pos, const std::string &origID, MSVehicle &v,
    SUMOReal &bestDistance, MSLane **lane, SUMOReal &lanePos, int &routeOffset,
    ConstMSEdgeVector & /*edges*/) {

  const ConstMSEdgeVector &edges = v.getRoute().getEdges();
  routeOffset = 0;
  // routes may be looped which makes routeOffset ambiguous. We first try to
  // find the closest upcoming edge on the route and then look for closer
  // passed
  // edges

  // look forward along the route
  const MSEdge *prev = 0;
  UNUSED_PARAMETER(prev); // silence 'unused variable' warning when built
                          // without INTERNAL_LANES
  for (ConstMSEdgeVector::const_iterator i = v.getCurrentRouteEdge();
       i != edges.end(); ++i) {
  ...
  UNUSED_PARAMETER(next); // silence 'unused variable' warning when built
                          // without INTERNAL_LANES
  for (ConstMSEdgeVector::const_iterator i = v.getCurrentRouteEdge();
       i != edges.begin(); --i) {
    prev = *i;
  ...
  // a) is enabled for non-internal lanes only, as otherwise the position
  // information may ambiguous
  // b) it's something one has to enable when building the nework - keepin the
  // OSM IDs - is probably not always done
  ...
  lanePos =
      MAX2(SUMOReal(0),
           MIN2(SUMOReal((*lane)->getLength() - POSITION_EPS),
                (*lane)->getShape().nearest_offset_to_point2D(pos, false)));
// std::cout << SIMTIME << " vtdMap_matchingRoutePosition vehicle=" << v.getID()
// << " currLane=" << v.getLane()->getID() << " routeOffset=" << routeOffset <<
// " edges=" << toString(edges) << " lane=" << (*lane)->getID() << "\n";
#ifdef DEBUG_VTD
  std::cout << "  b ok lane " << (*lane)->getID() << " lanePos:" << lanePos
            << std::endl;
#endif
  return true;
}

bool TraCIServerAPI_Vehicle::commandDistanceRequest(
    TraCIServer &server, tcpip::Storage &inputStorage,
    tcpip::Storage &outputStorage, const MSVehicle *v) {
  if (inputStorage.readUnsignedByte() != TYPE_COMPOUND) {
    return server.writeErrorStatusCmd(
        CMD_GET_VEHICLE_VARIABLE,
        "Retrieval of distance requires a compound object.", outputStorage);
  }
  if (inputStorage.readInt() != 2) {
    return server.writeErrorStatusCmd(CMD_GET_VEHICLE_VARIABLE,
                                      "Retrieval of distance requires position "
                                      "and distance type as parameter.",
                                      outputStorage);
  }
  ...
  case POSITION_ROADMAP:
    try {
      ...
      roadPos.first = TraCIServerAPI_Simulation::getLaneChecking(
          roadID, inputStorage.readUnsignedByte(), roadPos.second);
      pos = roadPos.first->getShape().positionAtOffset(roadPos.second);
    } catch (TraCIException &e) {
      return server.writeErrorStatusCmd(CMD_GET_VEHICLE_VARIABLE, e.what(),
                                        outputStorage);
    }
    break;
  case POSITION_2D:
  ...
  default:
    return server.writeErrorStatusCmd(
        CMD_GET_VEHICLE_VARIABLE,
        "Unknown position format used for distance request", outputStorage);
  }
  ...
  if (v->isOnRoad()) {
    if (distType == REQUEST_DRIVINGDIST) {
      distance = v->getRoute().getDistanceBetween(v->getPositionOnLane(),
                                                  roadPos.second, v->getEdge(),
                                                  &roadPos.first->getEdge());
      if (distance == std::numeric_limits<SUMOReal>::max()) {
        distance = INVALID_DOUBLE_VALUE;
      }
    } else {
...
// ------ helper functions ------
bool TraCIServerAPI_Vehicle::getPosition(const std::string &id, Position &p) {
  MSVehicle *v = dynamic_cast<MSVehicle *>(
      MSNet::getInstance()->getVehicleControl().getVehicle(id));
  if (v == 0) {
    return false;
  }
...
MSVehicleType &TraCIServerAPI_Vehicle::getSingularType(SUMOVehicle *const veh) {
  ...
  std::string newID = oType.getID().find('@') == std::string::npos
                          ? oType.getID() + "@" + veh->getID()
                          : oType.getID();
...
const std::map<std::string, std::vector<MSLane *> > &
  ...
  if (gVTDMap.size() == 0) {
    const MSEdgeVector &edges =
        MSNet::getInstance()->getEdgeControl().getEdges();
    for (MSEdgeVector::const_iterator i = edges.begin(); i != edges.end();
         ++i) {
      const std::vector<MSLane *> &lanes = (*i)->getLanes();
      for (std::vector<MSLane *>::const_iterator j = lanes.begin();
           j != lanes.end(); ++j) {
