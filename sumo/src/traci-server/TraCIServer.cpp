...
TraCIServer::TraCIServer(const SUMOTime begin, const int port)
    : mySocket(0), myTargetTime(begin), myDoingSimStep(false),
      myAmEmbedded(port == 0), myLaneTree(0) {

  myVehicleStateChanges[MSNet::VEHICLE_STATE_BUILT] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_DEPARTED] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_STARTING_TELEPORT] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_ENDING_TELEPORT] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_ARRIVED] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_NEWROUTE] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_STARTING_PARKING] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_ENDING_PARKING] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_STARTING_STOP] =
      std::vector<std::string>();
  myVehicleStateChanges[MSNet::VEHICLE_STATE_ENDING_STOP] =
      std::vector<std::string>();
  MSNet::getInstance()->addVehicleStateListener(this);

  myExecutors[CMD_GET_INDUCTIONLOOP_VARIABLE] =
      &TraCIServerAPI_InductionLoop::processGet;
  myExecutors[CMD_GET_AREAL_DETECTOR_VARIABLE] =
      &TraCIServerAPI_ArealDetector::processGet;
  myExecutors[CMD_GET_MULTI_ENTRY_EXIT_DETECTOR_VARIABLE] =
  ...
  myExecutors[CMD_SET_GET_VEHICLE_VARIABLE] =
      &TraCIServerAPI_Vehicle::processSetGet;
  myExecutors[CMD_GET_VEHICLETYPE_VARIABLE] =
      &TraCIServerAPI_VehicleType::processGet;
  myExecutors[CMD_SET_VEHICLETYPE_VARIABLE] =
  ...
  if (!MSGlobals::gUsingInternalLanes) {
    ...
    MsgHandler::getWarningInstance()->inform(
        "Vehicles will jump over junctions.", false);
    MsgHandler::getWarningInstance()->inform(
...
TraCIServer::~TraCIServer() {
  ...
  for (std::map<int, NamedRTree *>::const_iterator i = myObjects.begin();
       i != myObjects.end(); ++i) {
    delete (*i).second;
  }
  delete myLaneTree;
...
void TraCIServer::openSocket(const std::map<int, CmdExecutor> &execs) {
  ...
  if (!myDoCloseConnection &&
      OptionsCont::getOptions().getInt("remote-port") != 0) {
    myInstance = new TraCIServer(
        string2time(OptionsCont::getOptions().getString("begin")),
        OptionsCont::getOptions().getInt("remote-port"));
    for (std::map<int, CmdExecutor>::const_iterator i = execs.begin();
         i != execs.end(); ++i) {
...
bool TraCIServer::wasClosed() { return myDoCloseConnection; }

void TraCIServer::setVTDControlled(MSVehicle *v, MSLane *l, SUMOReal pos,
                                   SUMOReal angle, int edgeOffset,
                                   ConstMSEdgeVector route, SUMOTime t) {
...
void TraCIServer::postProcessVTD() {
  for (std::map<std::string, MSVehicle *>::const_iterator i =
           myVTDControlledVehicles.begin();
       i != myVTDControlledVehicles.end(); ++i) {
    if (MSNet::getInstance()->getVehicleControl().getVehicle((*i).first) != 0) {
      (*i).second->getInfluencer().postProcessVTD((*i).second);
    } else {
      WRITE_WARNING("Vehicle '" + (*i).first +
...
void TraCIServer::processCommandsUntilSimStep(SUMOTime step) {
  ...
  try {
    ...
    while (!myDoCloseConnection) {
      ...
      while (myInstance->myInputStorage.valid_pos() && !myDoCloseConnection) {
        ...
        if (cmd == CMD_SIMSTEP2) {
          ...
          // clang-format off
          for (std::map<MSNet::VehicleState, std::vector<std::string> >::iterator
               // clang-format on
               i = myInstance->myVehicleStateChanges.begin();
               i != myInstance->myVehicleStateChanges.end(); ++i) {
            (*i).second.clear();
          }
          return;
        }
    ...
    // clang-format off
    for (std::map<MSNet::VehicleState, std::vector<std::string> >::iterator i =
             // clang-format on
         myInstance->myVehicleStateChanges.begin();
         i != myInstance->myVehicleStateChanges.end(); ++i) {
      (*i).second.clear();
    }
...
// ===========================================================================
static PyObject *traciemb_execute(PyObject * /* self */, PyObject *args) {
  const char *msg;
  int size;
...
std::string TraCIServer::execute(std::string cmd) {
  try {
    if (myInstance == 0) {
      if (!myDoCloseConnection) {
        myInstance = new TraCIServer(
    ...
    return std::string(myInstance->myOutputStorage.begin(),
                       myInstance->myOutputStorage.end());
  } catch (std::invalid_argument &e) {
    throw ProcessError(e.what());
  } catch (TraCIException &e) {
    throw ProcessError(e.what());
...
void TraCIServer::runEmbedded(std::string pyFile) {
...
int TraCIServer::dispatchCommand() {
  ...
  if (myExecutors.find(commandId) != myExecutors.end()) {
    ...
    case CMD_SUBSCRIBE_GUI_CONTEXT:
      ...
      break;
    default:
      writeStatusCmd(commandId, RTYPE_NOTIMPLEMENTED,
                     "Command not implemented in sumo");
    }
  }
  if (!success) {
    while (myInputStorage.valid_pos() &&
           myInputStorage.position() < commandStart + commandLength) {
  ...
  if (myInputStorage.position() != commandStart + commandLength) {
    ...
    msg << " but " << myInputStorage.position() - commandStart
        << " Bytes were read.";
...
bool TraCIServer::commandGetVersion() {
  ...
  // When we get here, the response is stored in answerTmp -> put into
  // myOutputStorage
...
void TraCIServer::postProcessSimulationStep2() {
  ...
  int noActive = 0;
  for (std::vector<Subscription>::iterator i = mySubscriptions.begin();
       i != mySubscriptions.end();) {
    const Subscription &s = *i;
    bool isArrivedVehicle =
        (s.commandId == CMD_SUBSCRIBE_VEHICLE_VARIABLE ||
         s.commandId == CMD_SUBSCRIBE_VEHICLE_CONTEXT) &&
        (find(myVehicleStateChanges[MSNet::VEHICLE_STATE_ARRIVED].begin(),
              myVehicleStateChanges[MSNet::VEHICLE_STATE_ARRIVED].end(),
              s.id) !=
         myVehicleStateChanges[MSNet::VEHICLE_STATE_ARRIVED].end());
...
void TraCIServer::writeStatusCmd(int commandId, int status,
...
void TraCIServer::writeStatusCmd(int commandId, int status,
  ...
  if (status == RTYPE_ERR) {
    WRITE_ERROR("Answered with error to command " + toHex(commandId, 2) + ": " +
                description);
  } else if (status == RTYPE_NOTIMPLEMENTED) {
    WRITE_ERROR("Requested command not implemented (" + toHex(commandId, 2) +
                "): " + description);
  }
  outputStorage.writeUnsignedByte(
      1 + 1 + 1 + 4 + static_cast<int>(description.length())); // command length
...
void TraCIServer::initialiseSubscription(const TraCIServer::Subscription &s) {
  ...
  if (processSingleSubscription(s, writeInto, errors)) {
    ...
    writeStatusCmd(s.commandId, RTYPE_ERR,
                   "Could not add subscription (" + errors + ").");
  }
  myOutputStorage.writeStorage(writeInto);
}

void TraCIServer::removeSubscription(int commandId, const std::string &id,
                                     int domain) {
  bool found = false;
  for (std::vector<Subscription>::iterator j = mySubscriptions.begin();
       j != mySubscriptions.end();) {
    if ((*j).id == id && (*j).commandId == commandId &&
        (domain < 0 || (*j).contextDomain == domain)) {
  ...
  if (found) {
    ...
    writeStatusCmd(commandId, RTYPE_OK,
                   "The subscription to remove was not found.");
  }
}

bool TraCIServer::findObjectShape(int domain, const std::string &id,
...
void TraCIServer::collectObjectsInRange(int domain, const PositionVector &shape,
                                        SUMOReal range,
  ...
  if (myObjects.find(domain) == myObjects.end()) {
    ...
    case CMD_GET_INDUCTIONLOOP_VARIABLE:
      myObjects[CMD_GET_INDUCTIONLOOP_VARIABLE] =
...
bool TraCIServer::processSingleSubscription(const Subscription &s,
                                            tcpip::Storage &writeInto,
                                            std::string &errors) {
  bool ok = true;
  tcpip::Storage outputStorage;
  ...
  const int numVars =
      s.contextVars && s.variables.size() == 1 && s.variables[0] == ID_LIST
          ? 0
          : (int)s.variables.size();
  for (std::set<std::string>::iterator j = objIDs.begin(); j != objIDs.end();
       ++j) {
    if (s.contextVars) {
    ...
    if (numVars > 0) {
      // clang-format off
      std::vector<std::vector<unsigned char> >::const_iterator k =
          // clang-format on
          s.parameters.begin();
      for (std::vector<int>::const_iterator i = s.variables.begin();
           i != s.variables.end(); ++i, ++k) {
        ...
        if (myExecutors.find(getCommandId) != myExecutors.end()) {
          ...
          writeStatusCmd(s.commandId, RTYPE_NOTIMPLEMENTED,
                         "Unsupported command specified", tmpOutput);
          ok = false;
        }
        // copy response part
        if (ok) {
  ...
  unsigned int length =
      (1 + 4) + 1 + (4 + (int)(s.id.length())) + 1 + (int)outputStorage.size();
  if (s.contextVars) {
    length += 4;
  }
...
bool TraCIServer::addObjectVariableSubscription(const int commandId,
  ...
  Subscription s(commandId, id, variables, parameters, beginTime, endTime,
                 hasContext, domain, range);
...
void TraCIServer::writeResponseWithLength(tcpip::Storage &outputStorage,
  ...
  if (tempMsg.size() < 254) {
    outputStorage.writeUnsignedByte(
        1 + (int)tempMsg.size()); // command length -> short
  } else {
