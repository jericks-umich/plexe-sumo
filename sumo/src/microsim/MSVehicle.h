...
public:
   ...
   * @brief Stores the waiting intervals over the previous seconds (memory is to
   * be specified in ms.).
   */
  class WaitingTimeCollector {
    friend class MSVehicle;
  ...
  public:
    ...
    SUMOTime getMemorySize() const { return myMemorySize; }
  ...
  private:
    ...
    /// If the current (ongoing) waiting interval has begun at time t - dt
    /// (where t is the current time)
    /// then waitingIntervalList[0]->first = 0., waitingIntervalList[0]->second
    /// = dt
   ...
   *
   * @param[in] reason why the vehicle leaves (reached its destination, parking,
   * teleport)
   */
   ...
   */
  static inline SUMOReal gap(SUMOReal predPos, SUMOReal predLength,
                             SUMOReal pos) {
   ...
   *
   * This method goes through all active move reminder, both those for the
   * current
  ...
  /** @brief Compute safe velocities for the upcoming lanes based on positions
   * and
   * speeds from the last time step. Also registers
   * ApproachingVehicleInformation for all links
   *
   * This method goes through the best continuation lanes of the current lane
   * and
   * computes the safe velocities for passing/stopping at the next link as a
   * DriveProcessItem
   *
   ...
   */
  void planMove(const SUMOTime t, const MSVehicle *pred,
   ...
   * next lane (myLane is updated) if the end of the current lane is reached
   * (this may happen
   * multiple times in this method)
   *
   * The vehicle also sets the lanes it is in-lapping into and informs them
   * about it.
   ...
   */
  SUMOReal getPositionOnLane() const { return myState.myPos; }

  /** @brief Returns the vehicle's current speed
   * @return The vehicle's speed
   */
  SUMOReal getSpeed() const { return myState.mySpeed; }
  SUMOReal getSpeedKmph() const { return myState.mySpeed * 3.6; }
   ...
   */
  SUMOReal getAcceleration() const { return myAcceleration; }
  //@}
  ...
  /** @brief Returns the information whether the vehicle is on a road (is
   * simulated)
   * @return Whether the vehicle is simulated
   */
  inline bool isOnRoad() const { return myAmOnNet; }
   ...
   */
  SUMOTime getWaitingTime() const { return myWaitingTime; }

  /** @brief Returns the SUMOTime waited (speed was lesser than 0.1m/s) within
   * the last t millisecs
   *
   * @param[in] t specifies the length of the interval over which the cumulated
   * waiting time is to be summed up (defaults to and must not exceed
   * MSGlobals::gWaitingTimeMemory)
   * @return The time the vehicle was standing within the last t millisecs
   */
  SUMOTime
  getAccumulatedWaitingTime(SUMOTime t = MSGlobals::gWaitingTimeMemory) const {
   ...
   * @return The time the vehicle is standing
   */
  SUMOReal getWaitingSeconds() const { return STEPS2TIME(myWaitingTime); }

  /** @brief Returns the number of seconds waited (speed was lesser than 0.1m/s)
   * within the last millisecs
   *
   * @return The time the vehicle was standing within the last t millisecs
   */
   ...
   */
  SUMOReal getAngle() const { return myAngle; }
  //@}
  ...
  static bool overlap(const MSVehicle *veh1, const MSVehicle *veh2) {
    if (veh1->myState.myPos < veh2->myState.myPos) {
      return veh2->myState.myPos - veh2->getVehicleType().getLengthWithGap() <
             veh1->myState.myPos;
    }
    return veh1->myState.myPos - veh1->getVehicleType().getLengthWithGap() <
           veh2->myState.myPos;
  }
   ...
   * @param[in] onTeleporting Whether the lane was entered while being
   * teleported
   * @return Whether the vehicle's route has ended (due to vaporization, or
   * because the destination was reached)
   */
   ...
   * @param[in] speed The speed with which the vehicle was inserted into the
   * lane
   * @param[in] notification The cause of insertion (i.e. departure, teleport,
   * parking)
   */
   ...
   * all cfmodels work (some of them require veh->getLane() to return a valid
   * lane)
   * Once the vehicle is sucessfully inserted the lane is set again (see
  ...
  /** @brief Update of members if vehicle leaves a new lane in the lane change
   * step or at arrival. */
  ...
  struct LaneQ {
    ...
    MSLane *lane;
    /// @brief The overall length which may be driven when using this lane
    /// without a lane change
    SUMOReal length;
    /// @brief The length which may be driven on this lane
    SUMOReal currentLength;
    /// @brief The overall vehicle sum on consecutive lanes which can be passed
    /// without a lane change
    SUMOReal occupation;
    /// @brief As occupation, but without the first lane
    SUMOReal nextOccupation;
    /// @brief The (signed) number of lanes to be crossed to get to the lane
    /// which allows to continue the drive
    int bestLaneOffset;
    /// @brief Whether this lane allows to continue the drive
    bool allowsContinuation;
    /// @brief Consecutive lane that can be followed without a lane change
    /// (contribute to length and occupation)
    std::vector<MSLane *> bestContinuations;
  };

  /** @brief Returns the description of best lanes to use in order to continue
   * the route
   * @return The best lanes structure holding matching the current vehicle
   * position and state ahead
   */
   ...
   *  the container only.
   *
   * A rebuild must be done if the vehicle leaves a stop; then, another lane may
   * become
   *  the best one.
   *
   * If no starting lane ("startLane") is given, the vehicle's current lane
   * ("myLane")
   *  is used as start of bect lanes building.
   *
   * @param[in] forceRebuild Whether the best lanes container shall be rebuilt
   * even if the vehicle's edge has not changed
   * @param[in] startLane The lane the process shall start at ("myLane" will be
   * used if ==0)
   */
  void updateBestLanes(bool forceRebuild = false, const MSLane *startLane = 0);

  /** @brief Returns the subpart of best lanes that describes the vehicle's
   * current lane and their successors
   * @return The best lane information for the vehicle's current lane
   * @todo Describe better
   */
  const std::vector<MSLane *> &getBestLanesContinuation() const;

  /** @brief Returns the subpart of best lanes that describes the given lane and
   * their successors
   * @return The best lane information for the given lane
   * @todo Describe better
   */
  const std::vector<MSLane *> &
  getBestLanesContinuation(const MSLane *const l) const;
  ...
  /// @brief repair errors in vehicle position after changing between internal
  /// edges
  bool fixPosition();
   ...
   */
  bool addStop(const SUMOVehicleParameter::Stop &stopPar, std::string &errorMsg,
   ...
   */
  bool hasStops() const { return !myStops.empty(); }
  ...
  /** @brief return whether the given position is within range of the current
   * stop
   */
  ...
  /**
   * Compute distance that will be covered, if the vehicle moves to a given
   ...
   * @return      distance from the vehicles current position to the destination
   * position,
   *          or a near infinite real value if the destination position is not
   * contained
   *          within the vehicles route or the vehicle is not active
   */
  SUMOReal getDistanceToPosition(SUMOReal destPos,
   ...
   * @return The leading vehicle together with the gap; (0, -1) if no leader was
   * found.
   */
  std::pair<const MSVehicle *const, SUMOReal>
  getLeader(SUMOReal dist = 0) const;

  /** @brief Returns the time gap in seconds to the leader of the vehicle
   * looking for a fixed distance.
   *
  ...
  */
  ...
  enum LaneChangeMode {
    ...
    LC_NOCONFLICT =
        1, // lcModel may trigger changes if not in conflict with TraCI request
    LC_ALWAYS = 2 // lcModel may always trigger changes of this level regardless
                  // of requests
  };

  /// @brief modes for prioritizing traci lane change requests
  enum TraciLaneChangePriority {
    LCP_ALWAYS = 0, // change regardless of blockers, adapt own speed and speed
                    // of blockers
    LCP_NOOVERLAP = 1, // change unless overlapping with blockers, adapt own
                       // speed and speed of blockers
    LCP_URGENT =
        2, // change if not blocked, adapt own speed and speed of blockers
   ...
   */
  bool signalSet(int which) const { return (mySignals & which) != 0; }
  /// @}

  /// @brief whether the vehicle may safely move to the given lane with regard
  /// to upcoming links
   ...
   *  returned. Otherwise the speed returned from
   * myInfluencer->getOriginalSpeed().
  ...
  /**
   * schedule a new stop for the vehicle; each time a stop is reached, the
   * vehicle
   ...
   * @param parking  a flag indicating whether the traci stop is used for
   * parking or not
   * @param triggered a flag indicating whether the traci stop is triggered or
   * not
   * @param containerTriggered a flag indicating whether the traci stop is
   * triggered by a container or not
   */
  bool addTraciStop(MSLane *const lane, const SUMOReal startPos,
                    const SUMOReal endPos, const SUMOTime duration,
                    const SUMOTime until, const bool parking,
                    const bool triggered, const bool containerTriggered,
                    std::string &errorMsg);
   ...
   * @param containerTriggered a flag indicating whether the traci stop is
   * triggered by a container or not
   * @param isContainerStop a flag indicating whether the stop is a container
   * stop
   */
  bool addTraciBusOrContainerStop(const std::string &stopId,
                                  const SUMOTime duration, const SUMOTime until,
                                  const bool parking, const bool triggered,
  ...
  * @return true on success, the resuming fails if the vehicle wasn't parking in
  * the first place
  */
   ...
   *
   * The class is used for passing velocities or velocity profiles obtained via
   * TraCI to the vehicle.
   * The speed adaptation is controlled by the stored speedTimeLine
   * Additionally, the variables myConsiderSafeVelocity,
   * myConsiderMaxAcceleration, and myConsiderMaxDeceleration
   * control whether the safe velocity, the maximum acceleration, and the
   * maximum deceleration
   * have to be regarded.
   *
   * Furthermore this class is used to affect lane changing decisions according
   * to
  ...
  public:
     ...
     *
     * The given speed is assumed to be the non-influenced speed from
     ...
     */
    SUMOReal influenceSpeed(SUMOTime currentTime, SUMOReal speed,
                            SUMOReal vSafe, SUMOReal vMin, SUMOReal vMax);

    /** @brief Applies stored LaneChangeMode information and laneTimeLine
     * @param[in] currentTime The current simulation time
     * @param[in] currentEdge The current edge the vehicle is on
     * @param[in] currentLaneIndex The index of the lane the vehicle is
     * currently on
     * @param[in] state The LaneChangeAction flags as computed by the
     * laneChangeModel
     * @return The new LaneChangeAction flags to use
     */
    int influenceChangeDecision(const SUMOTime currentTime,
     ...
     */
    inline SUMOReal getOriginalSpeed() const { return myOriginalSpeed; }

    void setVTDControlled(MSLane *l, SUMOReal pos, SUMOReal angle,
                          int edgeOffset, const ConstMSEdgeVector &route,
                          SUMOTime t);

    SUMOTime getLastAccessTimeStep() const { return myLastVTDAccess; }
    ...
    /// @brief return the change in longitudinal position that is implicit in
    /// the new VTD position
  ...
  private:
    ...
    /// @name Flags for managing conflicts between the laneChangeModel and TraCI
    /// laneTimeLine
    //@{
  ...
  bool hasInfluencer() const { return myInfluencer != 0; }
  ...
  SUMOReal
  getSafeFollowSpeed(const std::pair<const MSVehicle *, SUMOReal> leaderInfo,
...
protected:
   ...
   *
   * All offsets already stored in "myOldLaneMoveReminderOffsets" are increased
   * by the
   *  length that has been left. All still active move reminders from
   * "myMoveReminders"
   *  are put into "myOldLaneMoveReminders" and the offset to the last lane is
   * added to
  ...
  /// @brief Whether the vehicle is on the network (not parking, teleported,
  /// vaporized, or arrived)
  bool myAmOnNet;

  /// @brief Whether this vehicle is registered as waiting for a person (for
  /// deadlock-recognition)
  bool myAmRegisteredAsWaitingForPerson;

  /// @brief Whether this vehicle is registered as waiting for a container (for
  /// deadlock-recognition)
...
protected:
  struct DriveProcessItem {
    ...
    DriveProcessItem(MSLink *link, SUMOReal vPass, SUMOReal vWait,
                     bool setRequest, SUMOTime arrivalTime,
                     SUMOReal arrivalSpeed, SUMOTime arrivalTimeBraking,
                     SUMOReal arrivalSpeedBraking, SUMOReal distance,
        ...
        : myLink(link), myVLinkPass(vPass), myVLinkWait(vWait),
          ...
          myArrivalTimeBraking(arrivalTimeBraking),
          myArrivalSpeedBraking(arrivalSpeedBraking), myDistance(distance),
    ...
    DriveProcessItem(SUMOReal vWait, SUMOReal distance)
        : myLink(0), myVLinkPass(vWait), myVLinkWait(vWait),
          mySetRequest(false), myArrivalTime(0), myArrivalSpeed(0),
          myArrivalTimeBraking(0), myArrivalSpeedBraking(0),
          myDistance(distance), accelV(-1), hadVehicle(false),
          availableSpace(-1.) {
      assert(vWait >= 0);
    };
  ...
  void planMoveInternal(const SUMOTime t, const MSVehicle *pred,
                        DriveItemVector &lfLinks) const;
  void checkRewindLinkLanes(const SUMOReal lengthsInFront,
                            DriveItemVector &lfLinks) const;

  /// @brief estimate leaving speed when accelerating across a link
  inline SUMOReal estimateLeaveSpeed(const MSLink *const link,
                                     const SUMOReal vLinkPass) const {
   ...
   * XXX affected by ticket #860 (the formula is invalid for the current
   * position update rule)
   */
  inline SUMOReal estimateSpeedAfterDistance(const SUMOReal dist,
                                             const SUMOReal v,
                                             const SUMOReal accel) const {
   ...
   * - a leader vehicle
   * - a vehicle or pedestrian that crosses this vehicles path on an upcoming
   * intersection
   ...
   * @param[in] distToCrossing The distance to the crossing point with the
   * current leader where relevant or -1
   */
...
private:
  /* @brief The vehicle's knowledge about edge efforts/travel times; @see
...
#ifndef NO_TRACI
  /// @brief An instance of a velocity/lane influencing instance; built in
  /// "getInfluencer"
