...
SUMOReal MSVehicle::State::pos() const { return myPos; }
...
MSVehicle::WaitingTimeCollector::WaitingTimeCollector(SUMOTime memory)
    : myMemorySize(memory) {}

MSVehicle::WaitingTimeCollector::WaitingTimeCollector(
    const WaitingTimeCollector &wt)
    : myMemorySize(wt.getMemorySize()),
      myWaitingIntervals(wt.getWaitingIntervals()) {}

MSVehicle::WaitingTimeCollector &MSVehicle::WaitingTimeCollector::
operator=(const WaitingTimeCollector &wt) {
  myMemorySize = wt.getMemorySize();
  myWaitingIntervals = wt.getWaitingIntervals();
  return *this;
}
...
SUMOTime MSVehicle::WaitingTimeCollector::cumulatedWaitingTime(
    SUMOTime memorySpan) const {
  assert(memorySpan <= myMemorySize);
  if (memorySpan == -1) {
    memorySpan = myMemorySize;
  }
  SUMOTime totalWaitingTime = 0;
  for (waitingIntervalList::const_iterator i = myWaitingIntervals.begin();
       i != myWaitingIntervals.end(); i++) {
...
void MSVehicle::WaitingTimeCollector::passTime(SUMOTime dt, bool waiting) {
...
MSVehicle::Influencer::Influencer()
    : mySpeedAdaptationStarted(true), myConsiderSafeVelocity(true),
      ...
      myRespectJunctionPriority(true), myEmergencyBrakeRedLight(true),
      myLastVTDAccess(-TIME2STEPS(20)), myStrategicLC(LC_NOCONFLICT),
      myCooperativeLC(LC_NOCONFLICT), mySpeedGainLC(LC_NOCONFLICT),
      myRightDriveLC(LC_NOCONFLICT), myTraciLaneChangePriority(LCP_URGENT) {}
...
SUMOReal MSVehicle::Influencer::influenceSpeed(SUMOTime currentTime,
                                               SUMOReal speed, SUMOReal vSafe,
                                               SUMOReal vMin, SUMOReal vMax) {
  // keep original speed
  myOriginalSpeed = speed;
  // remove leading commands which are no longer valid
  while (
      mySpeedTimeLine.size() == 1 ||
      (mySpeedTimeLine.size() > 1 && currentTime > mySpeedTimeLine[1].first)) {
...
int MSVehicle::Influencer::influenceChangeDecision(
    const SUMOTime currentTime, const MSEdge &currentEdge,
    const unsigned int currentLaneIndex, int state) {
  // remove leading commands which are no longer valid
  while (myLaneTimeLine.size() == 1 ||
         (myLaneTimeLine.size() > 1 && currentTime > myLaneTimeLine[1].first)) {
  ...
  if ((state & LCA_WANTS_LANECHANGE_OR_STAY) != 0) {
    ...
    if ((state & LCA_STRATEGIC) != 0) {
      ...
      WRITE_WARNING(
          "Lane change model did not provide a reason for changing (state=" +
          toString(state) + ", time=" + time2string(currentTime) + "\n");
    }
    if (mode == LC_NEVER) {
      ...
      if (((state & LCA_LEFT) != 0 && changeRequest != REQUEST_LEFT) ||
  ...
  if (changeRequest == REQUEST_NONE) {
    ...
    if ((myTraciLaneChangePriority == LCP_ALWAYS) ||
        (myTraciLaneChangePriority == LCP_NOOVERLAP &&
         (state & LCA_OVERLAPPING) == 0)) {
      state &= ~(LCA_BLOCKED | LCA_OVERLAPPING);
    }
    if (changeRequest != REQUEST_HOLD &&
        myTraciLaneChangePriority != LCP_OPPORTUNISTIC) {
      state |= LCA_URGENT;
    }
...
SUMOReal MSVehicle::Influencer::changeRequestRemainingSeconds(
    const SUMOTime currentTime) const {
...
void MSVehicle::Influencer::setLaneChangeMode(int value) {
  ...
  myTraciLaneChangePriority =
      (TraciLaneChangePriority)((value & (256 + 512)) >> 8);
}

void MSVehicle::Influencer::setVTDControlled(MSLane *l, SUMOReal pos,
                                             SUMOReal angle, int edgeOffset,
...
SUMOReal MSVehicle::Influencer::implicitSpeedVTD(const MSVehicle *veh,
                                                 SUMOReal oldSpeed) {
  const SUMOReal dist =
      veh->getDistanceToPosition(myVTDPos, &myVTDLane->getEdge());
...
MSVehicle::~MSVehicle() {
  ...
  delete myLaneChangeModel; // still needed when calling resetPartialOccupation
                            // (getShadowLane)
...
MSVehicle::MSVehicle(SUMOVehicleParameter *pars, const MSRoute *route,
    ...
    : MSBaseVehicle(pars, route, type, speedFactor), myWaitingTime(0),
      myWaitingTimeCollector(), myState(0, 0), //
      myLane(0), myLastBestLanesEdge(0), myLastBestLanesInternalLane(0),
      myPersonDevice(0), myContainerDevice(0), myAcceleration(0), mySignals(0),
      myAmOnNet(false), myAmRegisteredAsWaitingForPerson(false),
      myAmRegisteredAsWaitingForContainer(false), myHaveToWaitOnNextLink(false),
      myAngle(0), myCachedPosition(Position::INVALID), myEdgeWeights(0)
#ifndef NO_TRACI
      ,
      myInfluencer(0)
#endif
  ...
  if ((*myCurrEdge)->getPurpose() != MSEdge::EDGEFUNCTION_DISTRICT) {
    if (pars->departLaneProcedure == DEPART_LANE_GIVEN) {
      if ((*myCurrEdge)->getDepartLane(*this) == 0) {
        throw ProcessError("Invalid departlane definition for vehicle '" +
                           pars->id + "'.");
      }
    } else {
...
bool MSVehicle::replaceRoute(const MSRoute *newRoute, bool onInit, int offset) {
  const ConstMSEdgeVector &edges = newRoute->getEdges();
  // assert the vehicle may continue (must not be "teleported" or whatever to
  // another position)
  ...
  MSNet::getInstance()->informVehicleStateListener(
      this, MSNet::VEHICLE_STATE_NEWROUTE);
  // recheck old stops
  ...
  for (std::vector<SUMOVehicleParameter::Stop>::const_iterator i =
           newRoute->getStops().begin();
       i != newRoute->getStops().end(); ++i) {
    std::string error;
    addStop(*i, error);
    if (error != "") {
      WRITE_WARNING(error);
...
SUMOReal MSVehicle::getSlope() const {
  if (myLane == 0) {
    return 0;
  }
...
Position MSVehicle::getPosition(const SUMOReal offset) const {
  if (myLane == 0) {
  ...
  Position result =
      myLane->geometryPositionAtOffset(getPositionOnLane() + offset);
  if (changingLanes) {
    const Position other =
        getLaneChangeModel().getShadowLane()->geometryPositionAtOffset(
            getPositionOnLane() + offset);
    const SUMOReal dist = getLaneChangeModel().getLaneChangeCompletion() *
                          result.distanceTo(other);
...
const MSEdge *MSVehicle::getRerouteOrigin() const {
  ...
  if (myLane != 0 && (myCurrEdge + 1) != myRoute->end() &&
      myState.myPos >
          myLane->getLength() -
              getCarFollowModel().brakeGap(
                  myState.mySpeed, getCarFollowModel().getMaxDecel(), 0.)) {
    return *(myCurrEdge + 1);
  }
...
void MSVehicle::setAngle(SUMOReal angle) { myAngle = angle; }

SUMOReal MSVehicle::computeAngle() const {
  Position p1;
  Position p2;
  if (isParking()) {
    return myLane->getShape().rotationAtOffset(
        myLane->interpolateLanePosToGeometryPos(getPositionOnLane()));
  }
  if (getLaneChangeModel().isChangingLanes()) {
    // cannot use getPosition() because it already includes the offset to the
    // side and thus messes up the angle
  ...
  if (myState.myPos >= myType->getLength()) {
    ...
    p2 = myFurtherLanes.size() > 0
             ? myFurtherLanes.back()->geometryPositionAtOffset(
                   myFurtherLanes.back()->getPartialOccupatorEnd())
             : myLane->getShape().front();
    if (getLaneChangeModel().isChangingLanes() && myFurtherLanes.size() > 0 &&
        getLaneChangeModel().getShadowLane(myFurtherLanes.back()) == 0) {
  ...
  SUMOReal result =
      (p1 != p2
           ? p2.angleTo2D(p1)
           : myLane->getShape().rotationAtOffset(
                 myLane->interpolateLanePosToGeometryPos(getPositionOnLane())));
  if (getLaneChangeModel().isChangingLanes()) {
    const SUMOReal angleOffset =
        60 / STEPS2TIME(MSGlobals::gLaneChangeDuration) *
        (getLaneChangeModel().isLaneChangeMidpointPassed()
             ? 1 - getLaneChangeModel().getLaneChangeCompletion()
             : getLaneChangeModel().getLaneChangeCompletion());
    result +=
        getLaneChangeModel().getLaneChangeDirection() * DEG2RAD(angleOffset);
  }
  return result;
}
...
bool MSVehicle::addStop(const SUMOVehicleParameter::Stop &stopPar,
                        std::string &errorMsg, SUMOTime untilOffset) {
  Stop stop;
  stop.lane = MSLane::dictionary(stopPar.lane);
  if (!stop.lane->allowsVehicleClass(myType->getVehicleClass())) {
    errorMsg = "Vehicle '" + myParameter->id +
               "' is not allowed to stop on lane '" + stopPar.lane + "'.";
    return false;
  }
  stop.busstop = MSNet::getInstance()->getBusStop(stopPar.busstop);
  stop.containerstop =
      MSNet::getInstance()->getContainerStop(stopPar.containerstop);
  ...
  if (stop.startPos < 0 || stop.endPos > stop.lane->getLength()) {
    ...
    errorMsg += " for vehicle '" + myParameter->id + "' on lane '" +
                stopPar.lane + "' has an invalid position.";
    return false;
  }
  if (stop.busstop != 0 &&
      myType->getLength() / 2. > stop.endPos - stop.startPos) {
    errorMsg = "Bus stop '" + stop.busstop->getID() + "' on lane '" +
               stopPar.lane + "' is too short for vehicle '" + myParameter->id +
               "'.";
  }
  if (stop.containerstop != 0 &&
      myType->getLength() / 2. > stop.endPos - stop.startPos) {
    errorMsg = "Container stop '" + stop.containerstop->getID() +
               "' on lane '" + stopPar.lane + "' is too short for vehicle '" +
               myParameter->id + "'.";
  }
  ...
  if (stopPar.index == STOP_INDEX_END ||
    ...
    if (myStops.size() > 0) {
      ...
      if (prevStopEdge == stop.edge && prevStopPos > stop.endPos) {
        stop.edge =
            find(prevStopEdge + 1, myRoute->end(), &stop.lane->getEdge());
      }
    }
  } else {
  ...
  if (stop.edge == myRoute->end() || prevStopEdge > stop.edge ||
    ...
    errorMsg += " for vehicle '" + myParameter->id + "' on lane '" +
                stopPar.lane + "' is not downstream the current route.";
    return false;
  }
  // David.C:
  // if (!stop.parking && (myCurrEdge == stop.edge && myState.myPos >
  // stop.endPos - getCarFollowModel().brakeGap(myState.mySpeed))) {
  if (myCurrEdge == stop.edge &&
      myState.myPos >
          stop.endPos - getCarFollowModel().brakeGap(myState.mySpeed)) {
    errorMsg = "Stop for vehicle '" + myParameter->id + "' on lane '" +
               stopPar.lane + "' is too close to break.";
    return false;
  }
  if (!hasDeparted() && myCurrEdge == stop.edge) {
    ...
    if (myParameter->departPosProcedure == DEPART_POS_BASE ||
        myParameter->departPosProcedure == DEPART_POS_DEFAULT) {
      pos = MIN2(
          static_cast<SUMOReal>(getVehicleType().getLength() + POSITION_EPS),
          (*myCurrEdge)->getLength());
    }
    if (pos > stop.endPos) {
      if (stop.busstop != 0) {
        errorMsg = "Bus stop '" + stop.busstop->getID() + "'";
      ...
      errorMsg += " for vehicle '" + myParameter->id + "' on lane '" +
                  stopPar.lane + "' is before departPos.";
      return false;
    }
  }
  if (iter != myStops.begin()) {
    ...
    if (stop.until >= 0 && iter2->until > stop.until) {
      ...
      errorMsg += " for vehicle '" + myParameter->id + "' on lane '" +
                  stopPar.lane + "' ends earlier than previous stop.";
    }
  }
...
bool MSVehicle::isStopped() const {
  return !myStops.empty() &&
         myStops.begin()
             ->reached /*&& myState.mySpeed < SUMO_const_haltingSpeed @todo #1864#*/;
}
...
bool MSVehicle::isStoppedTriggered() const {
  return isStopped() &&
         (myStops.begin()->triggered || myStops.begin()->containerTriggered);
}

bool MSVehicle::isStoppedInRange(SUMOReal pos) const {
  return isStopped() && myStops.begin()->startPos <= pos &&
         myStops.begin()->endPos >= pos;
}
...
SUMOReal MSVehicle::processNextStop(SUMOReal currentVelocity) {
  ...
  if (stop.reached) {
    ...
    bool boarded = MSNet::getInstance()->getPersonControl().boardAnyWaiting(
        &myLane->getEdge(), this, &stop);
    boarded &= stop.awaitedPersons.size() == 0;
    // load containers
    bool loaded = MSNet::getInstance()->getContainerControl().loadAnyWaiting(
        &myLane->getEdge(), this, &stop);
    loaded &= stop.awaitedContainers.size() == 0;
    if (boarded) {
      if (stop.busstop != 0) {
        const std::vector<MSTransportable *> &persons =
            myPersonDevice->getTransportables();
        for (std::vector<MSTransportable *>::const_iterator i = persons.begin();
             i != persons.end(); ++i) {
          stop.busstop->removeTransportable(*i);
        }
      }
      // the triggering condition has been fulfilled. Maybe we want to wait a
      // bit longer for additional riders (car pooling)
    ...
    if (loaded) {
      if (stop.containerstop != 0) {
        const std::vector<MSTransportable *> &containers =
            myContainerDevice->getTransportables();
        for (std::vector<MSTransportable *>::const_iterator i =
                 containers.begin();
             i != containers.end(); ++i) {
    ...
    if (stop.duration <= 0 && !stop.triggered && !stop.containerTriggered) {
      ...
      if (stop.triggered && !myAmRegisteredAsWaitingForPerson) {
        // we can only register after waiting for one step. otherwise we might
        // falsely signal a deadlock
      ...
      if (stop.containerTriggered && !myAmRegisteredAsWaitingForContainer) {
        ...
        MSNet::getInstance()
            ->getVehicleControl()
            .registerOneWaitingForContainer();
    ...
    if (stop.edge == myCurrEdge) {
      ...
      if (stop.busstop != 0) {
        ...
        if (endPos != stop.busstop->getEndLanePosition() &&
            endPos - myType->getLength() / 2. <
                stop.busstop->getBeginLanePosition()) {
          busStopsMustHaveSpace = false;
        }
      }
      bool containerStopsMustHaveSpace = true;
      // if the stop is a container stop we check if the vehicle fits into the
      // last free position of the stop
      if (stop.containerstop != 0) {
        // on container stops, we have to wait for free place if they are in
        // use...
        endPos = stop.containerstop->getLastFreePos(*this);
        if (endPos != stop.containerstop->getEndLanePosition() &&
            endPos - myType->getLength() / 2. <
                stop.containerstop->getBeginLanePosition()) {
          containerStopsMustHaveSpace = false;
        }
      }
      // we use the same offset for container stops as for bus stops. we might
      // have to change it at some point!
      if (myState.pos() + getVehicleType().getMinGap() >=
              endPos - BUS_STOP_OFFSET &&
          busStopsMustHaveSpace && containerStopsMustHaveSpace &&
          myLane == stop.lane) {
        // ok, we may stop (have reached the stop)
        stop.reached = true;
        MSNet::getInstance()->getVehicleControl().addWaiting(&myLane->getEdge(),
                                                             this);
        MSNet::getInstance()->informVehicleStateListener(
            this, MSNet::VEHICLE_STATE_STARTING_STOP);
        // compute stopping time
        if (stop.until >= 0) {
          if (stop.duration == -1) {
            stop.duration =
                stop.until - MSNet::getInstance()->getCurrentTimeStep();
          } else {
            stop.duration =
                MAX2(stop.duration,
                     stop.until - MSNet::getInstance()->getCurrentTimeStep());
        ...
        if (stop.busstop != 0) {
          ...
          stop.busstop->enter(this,
                              myState.pos() + getVehicleType().getMinGap(),
                              myState.pos() - myType->getLength());
        }
        if (stop.containerstop != 0) {
          // let the container stop know the vehicle
          stop.containerstop->enter(this, myState.pos() +
                                              getVehicleType().getMinGap(),
                                    myState.pos() - myType->getLength());
        }
      }
      // decelerate
      return getCarFollowModel().stopSpeed(this, getSpeed(),
                                           endPos - myState.pos());
    }
  }
  return currentVelocity;
...
const ConstMSEdgeVector MSVehicle::getStopEdges() const {
  ...
  for (std::list<Stop>::const_iterator iter = myStops.begin();
...
void MSVehicle::planMove(const SUMOTime t, const MSVehicle *pred,
...
void MSVehicle::planMoveInternal(const SUMOTime t, const MSVehicle *pred,
...
#endif
  // remove information about approaching links, will be reset later in this
  // step
  ...
  if (myInfluencer != 0) {
    const SUMOReal vMin =
        MAX2(SUMOReal(0), cfModel.getSpeedAfterMaxDecel(myState.mySpeed));
    v = myInfluencer->influenceSpeed(MSNet::getInstance()->getCurrentTimeStep(),
                                     v, v, vMin, maxV);
  }
#endif
...
#endif
  SUMOReal seen =
      myLane->getLength() - myState.myPos; // the distance already "seen"; in
                                           // the following always up to the end
                                           // of the current "lane"
  SUMOReal seenNonInternal = 0;
  SUMOReal vLinkPass =
      MIN2(estimateSpeedAfterDistance(
               seen, v, getVehicleType().getCarFollowModel().getMaxAccel()),
           laneMaxV); // upper bound
  unsigned int view = 0;
  DriveProcessItem *lastLink = 0;
  bool slowedDownForMinor = false; // whether the vehicle already had to slow
                                   // down on approach to a minor link
  SUMOReal gap = 0;
  if (pred != 0) {
    if (pred == myLane->getPartialOccupator()) {
      gap = myLane->getPartialOccupatorEnd() - myState.myPos -
            getVehicleType().getMinGap();
    } else {
      gap = pred->getPositionOnLane() - pred->getVehicleType().getLength() -
            myState.myPos - getVehicleType().getMinGap();
    }
  }
  std::pair<const MSVehicle *, SUMOReal> leaderInfo = std::make_pair(pred, gap);
  // iterator over subsequent lanes and fill lfLinks until stopping distance or
  // stopped
  ...
  while (true) {
    ...
    if (getLaneChangeModel().hasShadowVehicle()) {
      ...
      if (shadowLane != 0) {
        std::pair<const MSVehicle *, SUMOReal> shadowLeaderInfo =
            shadowLane->getLeader(this, lane->getLength() - seen, false);
        adaptToLeader(shadowLeaderInfo, seen, lastLink, shadowLane, v,
                      vLinkPass);
      }
    }
    ...
    if (!myStops.empty() &&
      ...
      const SUMOReal endPos =
          stop.busstop == 0 ? stop.endPos : stop.busstop->getLastFreePos(*this);
    ...
    MSLinkCont::const_iterator link =
        MSLane::succLinkSec(*this, view + 1, *lane, bestLaneConts);
    //  check whether the vehicle is on its final edge
    if (myCurrEdge + view + 1 == myRoute->end()) {
      const SUMOReal arrivalSpeed =
          (myParameter->arrivalSpeedProcedure == ARRIVAL_SPEED_GIVEN
               ? myParameter->arrivalSpeed
               : laneMaxV);
      // subtract the arrival speed from the remaining distance so we get one
      // additional driving step with arrival speed
      const SUMOReal distToArrival =
          seen + myArrivalPos - lane->getLength() - SPEED2DIST(arrivalSpeed);
      const SUMOReal va =
          cfModel.freeSpeed(this, getSpeed(), distToArrival, arrivalSpeed);
      v = MIN2(v, va);
      if (lastLink != 0) {
    ...
    // We distinguish 3 cases when determining the point at which a vehicle
    // stops:
    // - links that require stopping: here the vehicle needs to stop close to
    // the stop line
    //   to ensure it gets onto the junction in the next step. Othwise the
    //   vehicle would 'forget'
    //   that it already stopped and need to stop again. This is necessary
    //   pending implementation of #999
    // - red/yellow light: here the vehicle 'knows' that it will have priority
    // eventually and does not need to stop on a precise spot
    // - other types of minor links: the vehicle needs to stop as close to the
    ...
    const SUMOReal laneStopOffset = yellowOrRed || (*link)->havePriority()
                                        ? DIST_TO_STOPLINE_EXPECT_PRIORITY
                                        : POSITION_EPS;
    const SUMOReal stopDist = MAX2(SUMOReal(0), seen - laneStopOffset);
    // check whether we need to slow down in order to finish a continuous lane
    // change
    if (getLaneChangeModel().isChangingLanes()) {
      if ( // slow down to finish lane change before a turn lane
          ((*link)->getDirection() == LINKDIR_LEFT ||
           (*link)->getDirection() == LINKDIR_RIGHT) ||
          // slow down to finish lane change before the shadow lane ends
          (getLaneChangeModel().isLaneChangeMidpointPassed() &&
           (*link)->getViaLaneOrLane()->getParallelLane(
               -getLaneChangeModel().getLaneChangeDirection()) == 0)) {
        // XXX maybe this is too harsh. Vehicles could cut some corners here
        const SUMOReal timeRemaining =
            STEPS2TIME((1 - getLaneChangeModel().getLaneChangeCompletion()) *
                       MSGlobals::gLaneChangeDuration);
        const SUMOReal va =
            MAX2((SUMOReal)0, (seen - POSITION_EPS) / timeRemaining);
        v = MIN2(va, v);
      }
    }

    // - always issue a request to leave the intersection we are currently on
    const bool leavingCurrentIntersection =
        myLane->getEdge().isInternal() && lastLink == 0;
    // - do not issue a request to enter an intersection after we already slowed
    // down for an earlier one
    const bool abortRequestAfterMinor =
        slowedDownForMinor && (*link)->getInternalLaneBefore() == 0;
    // - even if red, if we cannot break we should issue a request
    bool setRequest =
        (v > 0 && !abortRequestAfterMinor) || (leavingCurrentIntersection);

    SUMOReal vLinkWait = MIN2(v, cfModel.stopSpeed(this, getSpeed(), stopDist));
    const SUMOReal brakeDist = cfModel.brakeGap(myState.mySpeed) -
                               myState.mySpeed * cfModel.getHeadwayTime();
    if (yellowOrRed && seen >= brakeDist) {
      // the vehicle is able to brake in front of a yellow/red traffic light
      lfLinks.push_back(DriveProcessItem(
          *link, vLinkWait, vLinkWait, false,
          t + TIME2STEPS(seen / MAX2(vLinkWait, NUMERICAL_EPS)), vLinkWait, 0,
          0, seen));
      // lfLinks.push_back(DriveProcessItem(0, vLinkWait, vLinkWait, false, 0,
      // 0, stopDist));
      break;
    }
    ...
    if (MSGlobals::gUsingInternalLanes) {
      ...
      const MSLink::LinkLeaders linkLeaders =
          (*link)->getLeaderInfo(seen, getVehicleType().getMinGap());
      for (MSLink::LinkLeaders::const_iterator it = linkLeaders.begin();
           it != linkLeaders.end(); ++it) {
        // the vehicle to enter the junction first has priority
        const MSVehicle *leader = (*it).vehAndGap.first;
        if (leader == 0) {
          // leader is a pedestrian. Passing 'this' as a dummy.
          // std::cout << SIMTIME << " veh=" << getID() << " is blocked on link
          // to " << (*link)->getViaLaneOrLane()->getID() << " by pedestrian.
          // dist=" << it->distToCrossing << "\n";
          adaptToLeader(std::make_pair(this, -1), seen, lastLink, lane, v,
                        vLinkPass, it->distToCrossing);
        } else if ((*link)->isLeader(this, leader)) {
          adaptToLeader(it->vehAndGap, seen, lastLink, lane, v, vLinkPass,
                        it->distToCrossing);
          if (lastLink != 0) {
          ...
          if (v < SUMO_const_haltingSpeed &&
              leader->getLane()->getLogicalPredecessorLane() ==
                  myLane->getLogicalPredecessorLane()) {
            setRequest = false;
          }
        }
      }
      // if this is the link between two internal lanes we may have to slow down
      // for pedestrians
    ...
    // - unless they are close enough to have clear visibility and may start to
    // accelerate again
    // - and unless they are so close that stopping is impossible (i.e. when a
    // green light turns to yellow when close to the junction)
    if (!(*link)->havePriority() && stopDist > cfModel.getMaxDecel() &&
        brakeDist < seen) {
      // vehicle decelerates just enough to be able to stop if necessary and
      // then accelerates
      arrivalSpeed =
          MIN2(vLinkPass, cfModel.getMaxDecel() + cfModel.getMaxAccel());
      slowedDownForMinor = true;
    }
    // @note intuitively it would make sense to compare arrivalSpeed with
    // getSpeed() instead of v
    // however, due to the current position update rule (ticket #860) the
    // vehicle moves with v in this step
    const SUMOReal accel =
        (arrivalSpeed >= v) ? cfModel.getMaxAccel() : -cfModel.getMaxDecel();
    ...
    const SUMOReal nonAccelSpeed =
        MAX3(v, arrivalSpeed, SUMO_const_haltingSpeed);
    // subtract DELTA_T because t is the time at the end of this step and the
    // movement is not carried out yet
    const SUMOTime arrivalTime =
        t - DELTA_T + TIME2STEPS(accelTime + nonAccelWay / nonAccelSpeed);

    // compute speed, time if vehicle starts braking now
    // if stopping is possible, arrivalTime can be arbitrarily large. A small
    // value keeps fractional times (impatience) meaningful
    ...
    if (seen < cfModel.brakeGap(v)) {
      ...
      // Because we use a continuous formula for computiing the possible
      // slow-down
      // we need to handle the mismatch with the discrete dynamics
      if (seen < v) {
        arrivalSpeedBraking =
            arrivalSpeed; // no time left for braking after this step
      } else if (2 * (seen - v * cfModel.getHeadwayTime()) *
                         -cfModel.getMaxDecel() +
                     v * v >=
                 0) {
        arrivalSpeedBraking = estimateSpeedAfterDistance(
            seen - v * cfModel.getHeadwayTime(), v, -cfModel.getMaxDecel());
      } else {
        arrivalSpeedBraking = cfModel.getMaxDecel();
      }
      // due to discrecte/continuous mismatch we have to ensure that braking
      // actually helps
      arrivalSpeedBraking = MIN2(arrivalSpeedBraking, arrivalSpeed);
      arrivalTimeBraking =
          MAX2(arrivalTime,
               t + TIME2STEPS(seen / ((v + arrivalSpeedBraking) * 0.5)));
    }
    lfLinks.push_back(
        DriveProcessItem(*link, v, vLinkWait, setRequest, arrivalTime,
                         arrivalSpeed, arrivalTimeBraking, arrivalSpeedBraking,
                         seen, estimateLeaveSpeed(*link, arrivalSpeed)));
#ifdef HAVE_INTERNAL_LANES
    ...
    ++view;
#endif
    // we need to look ahead far enough to see available space for
    // checkRewindLinkLanes
    if ((!setRequest || v <= 0 || seen > dist) && hadNonInternal &&
        seenNonInternal > vehicleLength * CRLL_LOOK_AHEAD) {
      break;
    }
    ...
    // the link was passed
    // compute the velocity to use when the link is not blocked by other
    // vehicles
    //  the vehicle shall be not faster when reaching the next lane than allowed
    const SUMOReal va =
        MAX2(laneMaxV, cfModel.freeSpeed(this, getSpeed(), seen, laneMaxV));
    v = MIN2(va, v);
    seenNonInternal +=
        lane->getEdge().getPurpose() == MSEdge::EDGEFUNCTION_INTERNAL
            ? 0
            : lane->getLength();
    seen += lane->getLength();
    leaderInfo = lane->getLastVehicleInformation();
    leaderInfo.second = leaderInfo.second + seen - lane->getLength() -
                        getVehicleType().getMinGap();
    vLinkPass = MIN2(estimateSpeedAfterDistance(
                         lane->getLength(), v,
                         getVehicleType().getCarFollowModel().getMaxAccel()),
                     laneMaxV); // upper bound
    lastLink = &lfLinks.back();
  }
}

void MSVehicle::adaptToLeader(
...
SUMOReal MSVehicle::getSafeFollowSpeed(
    const std::pair<const MSVehicle *, SUMOReal> leaderInfo,
    const SUMOReal seen, const MSLane *const lane,
    SUMOReal distToCrossing) const {
  assert(leaderInfo.first != 0);
  const MSCFModel &cfModel = getCarFollowModel();
  SUMOReal vsafeLeader = 0;
  if (leaderInfo.second >= 0) {
    vsafeLeader = cfModel.followSpeed(
        this, getSpeed(), leaderInfo.second, leaderInfo.first->getSpeed(),
        leaderInfo.first->getCarFollowModel().getMaxDecel());
  } else {
    // the leading, in-lapping vehicle is occupying the complete next lane
    // stop before entering this lane
    vsafeLeader = cfModel.stopSpeed(this, getSpeed(),
                                    seen - lane->getLength() - POSITION_EPS);
  }
  if (distToCrossing >= 0) {
    // drive up to the crossing point with the current link leader
    vsafeLeader =
        MAX2(vsafeLeader, cfModel.stopSpeed(this, getSpeed(), distToCrossing));
  }
  return vsafeLeader;
}
...
bool MSVehicle::executeMove() {
  ...
  SUMOReal vSafeZipper =
      std::numeric_limits<SUMOReal>::max(); // speed limit due to zipper merging
  ...
  for (i = myLFLinkLanes.begin(); i != myLFLinkLanes.end(); ++i) {
    ...
    if (link != 0 && (*i).mySetRequest) {
      ...
      // vehicles should brake when running onto a yellow light if the distance
      // allows to halt in front
      const bool yellow =
          ls == LINKSTATE_TL_YELLOW_MAJOR || ls == LINKSTATE_TL_YELLOW_MINOR;
      const SUMOReal brakeGap =
          getCarFollowModel().brakeGap(myState.mySpeed) -
          getCarFollowModel().getHeadwayTime() * myState.mySpeed;
      if (yellow &&
          ((*i).myDistance > brakeGap ||
           myState.mySpeed < ACCEL2SPEED(getCarFollowModel().getMaxDecel()))) {
        vSafe = (*i).myVLinkWait;
...
//
...
#else
      const bool influencerPrio =
          (myInfluencer != 0 && !myInfluencer->getRespectJunctionPriority());
#endif
      std::vector<const SUMOVehicle *> collectFoes;
      const bool opened =
          yellow || influencerPrio ||
          link->opened((*i).myArrivalTime, (*i).myArrivalSpeed,
                       (*i).getLeaveSpeed(), getVehicleType().getLength(),
                       getImpatience(), getCarFollowModel().getMaxDecel(),
                       getWaitingTime(),
                       ls == LINKSTATE_ZIPPER ? &collectFoes : 0);
      // vehicles should decelerate when approaching a minor link
      if (opened && !influencerPrio && !link->havePriority() &&
          !link->lastWasContMajor() && !link->isCont()) {
        if ((*i).myDistance > getCarFollowModel().getMaxDecel()) {
          ...
          vSafeMin =
              MIN2((SUMOReal)DIST2SPEED(myLane->getLength() -
                                        getPositionOnLane() + POSITION_EPS),
                   (*i).myVLinkPass);
      ...
      if (opened) {
        vSafe = (*i).myVLinkPass;
        if (vSafe < getCarFollowModel().getMaxDecel() &&
            vSafe <= (*i).myVLinkWait &&
            vSafe < getCarFollowModel().maxNextSpeed(getSpeed(), this)) {
          // this vehicle is probably not gonna drive accross the next junction
          // (heuristic)
          myHaveToWaitOnNextLink = true;
        }
        ...
        vSafeZipper =
            MIN2(vSafeZipper,
                 link->getZipperSpeed(this, (*i).myDistance, (*i).myVLinkPass,
                                      (*i).myArrivalTime, &collectFoes));
      } else {
        vSafe = (*i).myVLinkWait;
        myHaveToWaitOnNextLink = true;
        if (ls == LINKSTATE_EQUAL) {
  ...
  // XXX braking due to lane-changing is not registered and due to processing
  // stops is not registered
  //     To avoid casual blinking brake lights at high speeds due to dawdling of
  //     the
  //      leading vehicle, we don't show brake lights when the deceleration
  //      could be caused
  //     by frictional forces and air resistance (i.e. proportional to v^2,
  //     coefficient could be adapted further)
  SUMOReal pseudoFriction = (0.05 + 0.005 * getSpeed()) * getSpeed();
  bool brakelightsOn = vSafe < getSpeed() - ACCEL2SPEED(pseudoFriction);
  // apply speed reduction due to dawdling / lane changing but ensure minimum
  // safe speed
  ...
  //    WRITE_WARNING("vehicle '" + getID() + "' cannot brake hard enough to
  //    reach safe speed "
  //            + toString(vSafe, 4) + ", moving at " + toString(vNext, 4) + "
  //            instead. time="
  ...
  if (myInfluencer != 0) {
    ...
    const SUMOReal vMax = getVehicleType().getCarFollowModel().maxNextSpeed(
        myState.mySpeed, this);
    const SUMOReal vMin = MAX2(
        SUMOReal(0), getVehicleType().getCarFollowModel().getSpeedAfterMaxDecel(
                         myState.mySpeed));
    vNext = myInfluencer->influenceSpeed(
        MSNet::getInstance()->getCurrentTimeStep(), vNext, vSafe, vMin, vMax);
  }
#endif
  // visit waiting time
    ...
    if (myCurrEdge != myRoute->end() - 1) {
      ...
      for (i = myLFLinkLanes.begin();
        ...
        if (approachedLane != myLane && approachedLane != 0) {
          ...
          if (getLaneChangeModel().isChangingLanes()) {
            if (link->getDirection() == LINKDIR_LEFT ||
                link->getDirection() == LINKDIR_RIGHT) {
              // abort lane change
              WRITE_WARNING(
                  "Vehicle '" + getID() + "' could not finish continuous lane "
                                          "change (turn lane) time=" +
                  time2string(MSNet::getInstance()->getCurrentTimeStep()) +
                  ".");
  ...
  if (!hasArrived() && !myLane->getEdge().isVaporizing()) {
    if (myState.myPos > myLane->getLength()) {
      WRITE_WARNING("Vehicle '" + getID() +
                    "' performs emergency stop at the end of lane '" +
                    myLane->getID() + "'" + emergencyReason + " (decel=" +
                    toString(myAcceleration - myState.mySpeed) + "), time=" +
                    time2string(MSNet::getInstance()->getCurrentTimeStep()) +
                    ".");
    ...
    // State needs to be reset for all vehicles before the next call to
...
SUMOReal MSVehicle::getSpaceTillLastStanding(const MSLane *l,
  ...
  for (MSLane::VehCont::const_iterator i = vehs.begin(); i != vehs.end(); ++i) {
    if ((*i)->getSpeed() < SUMO_const_haltingSpeed &&
        !(*i)->getLane()->getEdge().isRoundabout()) {
      foundStopped = true;
      const SUMOReal ret = (*i)->getPositionOnLane() -
...
void MSVehicle::checkRewindLinkLanes(const SUMOReal lengthsInFront,
  ...
  if (MSGlobals::gUsingInternalLanes && !myLane->getEdge().isRoundabout()) {
    ...
    // this is the sum of non-interal lane length minus in-between vehicle
    // lenghts
    for (unsigned int i = 0; i < lfLinks.size(); ++i) {
      ...
      if (approachedLane != 0) {
        if (item.myLink->hasFoes() &&
            item.myLink
                ->keepClear() /* && item.myLink->willHaveBlockedFoe()*/) {
          seenSpace = seenSpace - approachedLane->getBruttoVehLenSum();
          hadVehicle |= approachedLane->getVehicleNumber() != 0;
        } else {
          seenSpace = seenSpace + getSpaceTillLastStanding(approachedLane,
                                                           foundStopped); // -
          // approachedLane->getBruttoVehLenSum()
      ...
      if (last == 0) {
        ...
        if (last != 0) {
          ...
          hadVehicle = true;
          /// XXX spaceTillLastStanding should already be covered by
          /// getPartialOccupatorEnd()
          seenSpace = seenSpace + getSpaceTillLastStanding(approachedLane,
                                                           foundStopped); // -
          // approachedLane->getBruttoVehLenSum()
          // + approachedLane->getLength();
        ...
        if (last->signalSet(VEH_SIGNAL_BRAKELIGHT)) {
          const SUMOReal lastBrakeGap = last->getCarFollowModel().brakeGap(
              approachedLane->getLastVehicle()->getSpeed());
          const SUMOReal lastGap =
              last->getPositionOnLane() -
              last->getVehicleType().getLengthWithGap() + lastBrakeGap -
              last->getSpeed() * last->getCarFollowModel().getHeadwayTime();
          item.availableSpace = MAX2(seenSpace, seenSpace + lastGap);
          seenSpace += getSpaceTillLastStanding(
              approachedLane,
              foundStopped); // - approachedLane->getBruttoVehLenSum() +
                             // approachedLane->getLength();
        } else {
...
#endif
    // check which links allow continuation and add pass available to the
    // previous item
    for (int i = ((int)lfLinks.size() - 1); i > 0; --i) {
      DriveProcessItem &item = lfLinks[i - 1];
      const bool canLeaveJunction =
          item.myLink->getViaLane() == 0 || lfLinks[i].mySetRequest;
      const bool opened =
          item.myLink != 0 && canLeaveJunction &&
          (item.myLink->havePriority() ||
#ifndef NO_TRACI
           (myInfluencer != 0 && !myInfluencer->getRespectJunctionPriority()) ||
#endif
           item.myLink->opened(
               item.myArrivalTime, item.myArrivalSpeed, item.getLeaveSpeed(),
               getVehicleType().getLength(), getImpatience(),
               getCarFollowModel().getMaxDecel(), getWaitingTime()));
      bool allowsContinuation = item.myLink == 0 || item.myLink->isCont() ||
                                !lfLinks[i].hadVehicle || opened;
    ...
    for (unsigned int i = 0;
         hadVehicle && i < lfLinks.size() && removalBegin < 0; ++i) {
      // skip unset links
      ...
      /*
      SUMOReal impatienceCorrection = MAX2(SUMOReal(0),
      SUMOReal(SUMOReal(myWaitingTime)));
      if
      (seenSpace<getVehicleType().getLengthWithGap()-impatienceCorrection/10.&&nextSeenNonInternal!=0)
      {
          removalBegin = lastLinkToInternal;
      }
      */

      const SUMOReal leftSpace =
          item.availableSpace - getVehicleType().getLengthWithGap();
    ...
    if (removalBegin != -1 &&
      ...
      while (removalBegin < (int)(lfLinks.size())) {
        ...
        if (lfLinks[removalBegin].myDistance >= brakeGap ||
            (lfLinks[removalBegin].myDistance > 0 &&
             myState.mySpeed <
                 ACCEL2SPEED(getCarFollowModel().getMaxDecel()))) {
  ...
  for (DriveItemVector::iterator i = lfLinks.begin(); i != lfLinks.end(); ++i) {
    if ((*i).myLink != 0) {
      if ((*i).myLink->getState() == LINKSTATE_ALLWAY_STOP) {
        (*i).myArrivalTime +=
            (SUMOTime)RandHelper::rand((size_t)2); // tie braker
      }
      (*i).myLink->setApproaching(
          this, (*i).myArrivalTime, (*i).myArrivalSpeed, (*i).getLeaveSpeed(),
          (*i).mySetRequest, (*i).myArrivalTimeBraking,
          (*i).myArrivalSpeedBraking, getWaitingTime(), (*i).myDistance);
    }
  }
}
...
void MSVehicle::enterLaneAtInsertion(
  ...
  if (notification != MSMoveReminder::NOTIFICATION_TELEPORT) {
    // set and activate the new lane's reminders, teleports already did that at
    // enterLaneAtMove
    for (std::vector<MSMoveReminder *>::const_iterator rem =
             enteredLane->getMoveReminders().begin();
         rem != enteredLane->getMoveReminders().end(); ++rem) {
...
void MSVehicle::leaveLane(const MSMoveReminder::Notification reason) {
  ...
  if (reason != MSMoveReminder::NOTIFICATION_JUNCTION &&
      reason != MSMoveReminder::NOTIFICATION_LANE_CHANGE) {
  ...
  if (reason != MSMoveReminder::NOTIFICATION_PARKING &&
      reason != MSMoveReminder::NOTIFICATION_LANE_CHANGE) {
    while (!myStops.empty() && myStops.front().edge == myCurrEdge) {
      WRITE_WARNING("Vehicle '" + getID() + "' skips stop on lane '" +
                    myStops.front().lane->getID() + "' time=" +
                    time2string(MSNet::getInstance()->getCurrentTimeStep()) +
                    ".")
      myStops.pop_front();
    }
...
void MSVehicle::updateBestLanes(bool forceRebuild, const MSLane *startLane) {
#ifdef DEBUG_VEHICLE_GUI_SELECTION
  if (gDebugSelectedVehicle == getID()) {
    int bla = 0;
    myLastBestLanesEdge = 0;
  ...
  if (startLane->getEdge().getPurpose() == MSEdge::EDGEFUNCTION_INTERNAL) {
    if (myBestLanes.size() == 0 || forceRebuild) {
      // rebuilt from previous non-internal lane (may backtrack twice if behind
    ...
    // clang-format off
    for (std::vector<std::vector<LaneQ> >::iterator it = myBestLanes.begin();
         // clang-format on
         it != myBestLanes.end();) {
      std::vector<LaneQ> &lanes = *it;
      assert(lanes.size() > 0);
      if (&(lanes[0].lane->getEdge()) == nextEdge) {
        // keep those lanes which are successors of internal lanes from the edge
        // of startLane
        std::vector<LaneQ> oldLanes = lanes;
        lanes.clear();
        const std::vector<MSLane *> &sourceLanes =
            startLane->getEdge().getLanes();
        for (std::vector<MSLane *>::const_iterator it_source =
                 sourceLanes.begin();
             it_source != sourceLanes.end(); ++it_source) {
          for (std::vector<LaneQ>::iterator it_lane = oldLanes.begin();
               it_lane != oldLanes.end(); ++it_lane) {
        ...
        for (int i = 0; i < (int)lanes.size(); ++i) {
          ...
          if (lanes[i].bestContinuations[0] != 0) {
            // patch length of bestContinuation to match expectations (only
            // once)
            lanes[i].bestContinuations.insert(
                lanes[i].bestContinuations.begin(), (MSLane *)0);
          }
  ...
  if (nextStopEdge != 0) {
    ...
    nextStopPos =
        MAX2(POSITION_EPS,
             MIN2((SUMOReal)nextStopPos,
                  (SUMOReal)(nextStopEdge->getLength() - 2 * POSITION_EPS)));
  }
  ...
  for (MSRouteIterator ce = myCurrEdge; progress;) {
    ...
    for (std::vector<MSLane *>::const_iterator i = lanes.begin();
      ...
      q.bestLaneOffset = 0;
      q.length = cl->allowsVehicleClass(myType->getVehicleClass())
                     ? cl->getLength()
                     : 0;
      q.currentLength = q.length;
      q.allowsContinuation =
          allowed == 0 ||
          find(allowed->begin(), allowed->end(), cl) != allowed->end();
      q.occupation = 0;
      q.nextOccupation = 0;
  ...
  for (std::vector<std::vector<LaneQ> >::reverse_iterator i =
           // clang-format on
       myBestLanes.rbegin() + 1;
       i != myBestLanes.rend(); ++i) {
    ...
    // compute index of the best lane (highest length and least offset from the
    // best next lane)
    int bestThisIndex = 0;
    if (bestConnectedLength > 0) {
      index = 0;
      for (std::vector<LaneQ>::iterator j = clanes.begin(); j != clanes.end();
           ++j, ++index) {
        LaneQ bestConnectedNext;
        bestConnectedNext.length = -1;
        if ((*j).allowsContinuation) {
          for (std::vector<LaneQ>::const_iterator m = nextLanes.begin();
               m != nextLanes.end(); ++m) {
            if ((*m).lane->isApproachedFrom(&cE, (*j).lane)) {
              if (bestConnectedNext.length < (*m).length ||
                  (bestConnectedNext.length == (*m).length &&
                   abs(bestConnectedNext.bestLaneOffset) >
        ...
        if (clanes[bestThisIndex].length < (*j).length ||
            ...
            (clanes[bestThisIndex].length == (*j).length &&
             abs(clanes[bestThisIndex].bestLaneOffset) ==
                 abs((*j).bestLaneOffset) &&
             nextLinkPriority(clanes[bestThisIndex].bestContinuations) <
                 nextLinkPriority((*j).bestContinuations))) {
          bestThisIndex = index;
        }
      }
    ...
    for (std::vector<LaneQ>::iterator j = clanes.begin(); j != clanes.end();
      ...
      if ((*j).length < clanes[bestThisIndex].length ||
          ...
          (nextLinkPriority((*j).bestContinuations)) <
              nextLinkPriority(clanes[bestThisIndex].bestContinuations)) {
        (*j).bestLaneOffset = bestThisIndex - index;
        if ((nextLinkPriority((*j).bestContinuations)) <
            nextLinkPriority(clanes[bestThisIndex].bestContinuations)) {
...
MSVehicle::getBestLanesContinuation(const MSLane *const l) const {
  ...
  for (std::vector<LaneQ>::const_iterator i = myBestLanes[0].begin();
       i != myBestLanes[0].end(); ++i) {
    if ((*i).lane == lane) {
...
void MSVehicle::adaptBestLanesOccupation(int laneIndex, SUMOReal density) {
...
SUMOReal MSVehicle::getDistanceToPosition(SUMOReal destPos,
  ...
  if (isOnRoad() && destEdge != NULL) {
    if (&myLane->getEdge() == *myCurrEdge) {
      ...
      distance +=
          myRoute->getDistanceBetween(0, destPos, *(myCurrEdge + 1), destEdge);
    }
  }
  return distance;
}
...
MSVehicle::getLeader(SUMOReal dist) const {
  ...
  if (dist == 0) {
    dist =
        getCarFollowModel().brakeGap(getSpeed()) + getVehicleType().getMinGap();
  }
  const MSVehicle *lead = 0;
  const MSLane::VehCont &vehs = myLane->getVehiclesSecure();
  assert(vehs.size() > 0);
  MSLane::VehCont::const_iterator it =
      std::find(vehs.begin(), vehs.end(), this);
  if (it != vehs.end() && it + 1 != vehs.end()) {
    lead = *(it + 1);
  }
  if (lead != 0) {
    std::pair<const MSVehicle *const, SUMOReal> result(
        lead, lead->getPositionOnLane() - lead->getVehicleType().getLength() -
                  getPositionOnLane() - getVehicleType().getMinGap());
  ...
  return myLane->getLeaderOnConsecutive(dist, seen, getSpeed(), *this,
                                        bestLaneConts);
}
...
SUMOReal MSVehicle::getCO2Emissions() const {
  return PollutantsInterface::compute(myType->getEmissionClass(),
                                      PollutantsInterface::CO2, myState.speed(),
                                      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getCOEmissions() const {
  return PollutantsInterface::compute(myType->getEmissionClass(),
                                      PollutantsInterface::CO, myState.speed(),
                                      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getHCEmissions() const {
  return PollutantsInterface::compute(myType->getEmissionClass(),
                                      PollutantsInterface::HC, myState.speed(),
                                      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getNOxEmissions() const {
  return PollutantsInterface::compute(
      myType->getEmissionClass(), PollutantsInterface::NO_X, myState.speed(),
      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getPMxEmissions() const {
  return PollutantsInterface::compute(
      myType->getEmissionClass(), PollutantsInterface::PM_X, myState.speed(),
      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getFuelConsumption() const {
  return PollutantsInterface::compute(
      myType->getEmissionClass(), PollutantsInterface::FUEL, myState.speed(),
      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getElectricityConsumption() const {
  return PollutantsInterface::compute(
      myType->getEmissionClass(), PollutantsInterface::ELEC, myState.speed(),
      myAcceleration, getSlope());
}

SUMOReal MSVehicle::getHarmonoise_NoiseEmissions() const {
  return HelpersHarmonoise::computeNoise(myType->getEmissionClass(),
                                         myState.speed(), myAcceleration);
}

void MSVehicle::addPerson(MSTransportable *person) {
  if (myPersonDevice == 0) {
    myPersonDevice =
        MSDevice_Transportable::buildVehicleDevices(*this, myDevices, false);
  ...
  if (myStops.size() > 0 && myStops.front().reached &&
    ...
    unsigned int numExpected =
        (unsigned int)myStops.front().awaitedPersons.size();
    if (numExpected != 0) {
      // I added the if-statement and number retrieval, assuming that it should
      // be a "conditional short jump" only and
      //  in most cases we won't have the list of expected passenger - only for
      //  simulating car-sharing, probably.
...
void MSVehicle::addContainer(MSTransportable *container) {
  if (myContainerDevice == 0) {
    myContainerDevice =
        MSDevice_Transportable::buildVehicleDevices(*this, myDevices, true);
  ...
  if (myStops.size() > 0 && myStops.front().reached &&
    ...
    unsigned int numExpected =
        (unsigned int)myStops.front().awaitedContainers.size();
    if (numExpected != 0) {
...
unsigned int MSVehicle::getContainerNumber() const {
...
void MSVehicle::setBlinkerInformation() {
  ...
  if ((state & LCA_LEFT) != 0) {
    ...
    MSLinkCont::const_iterator link =
        MSLane::succLinkSec(*this, 1, *lane, getBestLanesContinuation());
    if (link != lane->getLinkCont().end() &&
        lane->getLength() - getPositionOnLane() <
            lane->getVehicleMaxSpeed(this) * (SUMOReal)7.) {
...
unsigned int MSVehicle::getLaneIndex() const {
  std::vector<MSLane *>::const_iterator laneP =
      std::find(myLane->getEdge().getLanes().begin(),
                myLane->getEdge().getLanes().end(), myLane);
  return (unsigned int)std::distance(myLane->getEdge().getLanes().begin(),
                                     laneP);
}

void MSVehicle::setTentativeLaneAndPosition(MSLane *lane, const SUMOReal pos) {
  assert(lane != 0);
  myLane = lane;
  myState.myPos = pos;
}
...
bool MSVehicle::unsafeLinkAhead(const MSLane *lane) const {
  ...
  // - zipper links if they are close enough and have approaching vehicles in
  // the relevant time range
  // - unprioritized links if the vehicle is currently approaching a prioritzed
  // link and unable to stop in time
  SUMOReal seen = myLane->getLength() - getPositionOnLane();
  const SUMOReal dist = getCarFollowModel().brakeGap(
      getSpeed(), getCarFollowModel().getMaxDecel(), 0);
  if (seen < dist) {
    const std::vector<MSLane *> &bestLaneConts = getBestLanesContinuation(lane);
    unsigned int view = 1;
    MSLinkCont::const_iterator link =
        MSLane::succLinkSec(*this, view, *lane, bestLaneConts);
    ...
    while (!lane->isLinkEnd(link) && seen <= dist) {
      if (!lane->getEdge().isInternal() &&
        ...
        if (found) {
          const SUMOTime leaveTime = (*link)->getLeaveTime(
              (*di).myArrivalTime, (*di).myArrivalSpeed, (*di).getLeaveSpeed(),
              getVehicleType().getLength());
          if ((*link)->hasApproachingFoe((*di).myArrivalTime, leaveTime,
                                         (*di).myArrivalSpeed,
                                         getCarFollowModel().getMaxDecel())) {
            // std::cout << SIMTIME << " veh=" << getID() << " aborting
            // changeTo=" << Named::getIDSecure(bestLaneConts.front()) << "
            // linkState=" << toString((*link)->getState()) << " seen=" << seen
            // << " dist=" << dist << "\n";
            return true;
          }
        }
...
#ifndef NO_TRACI
bool MSVehicle::addTraciStop(MSLane *const lane, const SUMOReal startPos,
                             const SUMOReal endPos, const SUMOTime duration,
                             const SUMOTime until, const bool parking,
...
bool MSVehicle::addTraciBusOrContainerStop(
    const std::string &stopId, const SUMOTime duration, const SUMOTime until,
    const bool parking, const bool triggered, const bool containerTriggered,
    const bool isContainerStop, std::string &errorMsg) {
  // if the stop exists update the duration
  for (std::list<Stop>::iterator iter = myStops.begin(); iter != myStops.end();
       iter++) {
    const Named *const stop =
        isContainerStop ? (Named *)iter->containerstop : iter->busstop;
  ...
  if (isContainerStop) {
    ...
    if (bs == 0) {
      errorMsg = "The container stop '" + stopId +
                 "' is not known for vehicle '" + getID() + "'";
      return false;
    }
  } else {
    ...
    if (bs == 0) {
      errorMsg = "The bus stop '" + stopId + "' is not known for vehicle '" +
                 getID() + "'";
      return false;
    }
  }
...
bool MSVehicle::resumeFromStopping() {
  if (isStopped()) {
    ...
    MSNet::getInstance()->getVehicleControl().removeWaiting(&myLane->getEdge(),
                                                            this);
    myStops.pop_front();
    ...
    // continue as wished...
    MSNet::getInstance()->informVehicleStateListener(
        this, MSNet::VEHICLE_STATE_ENDING_STOP);
    return true;
  }
  return false;
}

MSVehicle::Stop &MSVehicle::getNextStop() { return myStops.front(); }
...
int MSVehicle::influenceChangeDecision(int state) {
  if (hasInfluencer()) {
    state = getInfluencer().influenceChangeDecision(
        MSNet::getInstance()->getCurrentTimeStep(), myLane->getEdge(),
        getLaneIndex(), state);
  }
  return state;
}
...
void MSVehicle::saveState(OutputDevice &out) {
...
void MSVehicle::loadState(const SUMOSAXAttributes &attrs,
