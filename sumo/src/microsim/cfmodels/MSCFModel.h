...
public:
   ...
   */
  MSCFModel(const MSVehicleType *vtype, SUMOReal accel, SUMOReal decel,
   ...
   * @return The velocity after applying interactions with stops and lane change
   * model influences
   */
   ...
   *
   * Returns the velocity of the vehicle in dependence to the length of the free
   * street and the target
   *  velocity at the end of the free range. If onInsertion is true, the vehicle
   * may still brake
   ...
   */
  virtual SUMOReal freeSpeed(const MSVehicle *const veh, SUMOReal speed,
                             SUMOReal seen, SUMOReal maxSpeed,
                             const bool onInsertion = false) const;

  /** @brief Computes the vehicle's follow speed (no dawdling)
   *
   * Returns the velocity of the vehicle in dependence to the vehicle's and its
   * leader's values and the distance between them.
   * @param[in] veh The vehicle (EGO)
   * @param[in] speed The vehicle's speed
   ...
   * @return EGO's safe speed
   */
  virtual SUMOReal followSpeed(const MSVehicle *const veh, SUMOReal speed,
                               SUMOReal gap2pred, SUMOReal predSpeed,
                               SUMOReal predMaxDecel) const = 0;
   ...
   * @return EGO's safe speed
   */
  virtual SUMOReal insertionFollowSpeed(const MSVehicle *const veh,
                                        SUMOReal speed, SUMOReal gap2pred,
                                        SUMOReal predSpeed,
                                        SUMOReal predMaxDecel) const;

  /** @brief Computes the vehicle's safe speed for approaching a non-moving
   * obstacle (no dawdling)
   *
   * Returns the velocity of the vehicle when approaching a static object (such
   * as the end of a lane) assuming no reaction time is needed.
   * @param[in] veh The vehicle (EGO)
   * @param[in] gap2pred The (netto) distance to the the obstacle
   * @return EGO's safe speed for approaching a non-moving obstacle
   * @todo generic Interface, models can call for the values they need
   */
  virtual SUMOReal stopSpeed(const MSVehicle *const veh, const SUMOReal speed,
                             SUMOReal gap2pred) const = 0;

  /** @brief Returns the maximum gap at which an interaction between both
   * vehicles occurs
   *
   ...
   */
  virtual SUMOReal interactionGap(const MSVehicle *const veh,
                                  SUMOReal vL) const;
   ...
   */
  virtual VehicleVariables *createVehicleVariables() const { return 0; }
  /// @}
   ...
   */
  inline SUMOReal getMaxAccel() const { return myAccel; }
   ...
   */
  inline SUMOReal getMaxDecel() const { return myDecel; }
   ...
   */
  virtual SUMOReal getImperfection() const { return -1; }

  /** @brief Get the driver's reaction time [s]
   * @return The reaction time of this class' drivers in s
   */
  virtual SUMOReal getHeadwayTime() const { return myHeadwayTime; }
  /// @}
   ...
   *  duration.
   *
   * Justification: Due to air brake or other influences, the vehicle's next
   * maximum
   ...
   */
  virtual SUMOReal maxNextSpeed(SUMOReal speed,
                                const MSVehicle *const veh) const;

  /** @brief Returns the distance the vehicle needs to halt including driver's
   * reaction time
   * @param[in] speed The vehicle's current speed
  ...
  inline static SUMOReal brakeGap(const SUMOReal speed, const SUMOReal decel,
                                  const SUMOReal headwayTime) {
    /* one possiblity to speed this up is to precalculate speedReduction * steps
    * (steps+1) / 2
    ...
    return SPEED2DIST(steps * speed -
                      speedReduction * steps * (steps + 1) / 2) +
           speed * headwayTime;
  }

  inline static SUMOReal freeSpeed(const SUMOReal decel, const SUMOReal seen,
    ...
    const SUMOReal y = MAX2(
        0.0,
        ((sqrt((b + 2.0 * v) * (b + 2.0 * v) + 8.0 * b * seen) - b) * 0.5 - v) /
            b);
    const SUMOReal yFull = floor(y);
    const SUMOReal exactGap =
        (yFull * yFull + yFull) * 0.5 * b + yFull * v + (y > yFull ? v : 0.0);
    const SUMOReal fullSpeedGain =
        (yFull + (onInsertion ? 1. : 0.)) * ACCEL2SPEED(decel);
    return DIST2SPEED(MAX2((SUMOReal)0.0, seen - exactGap) / (yFull + 1)) +
           fullSpeedGain + maxSpeed;
  }

  /** @brief Returns the minimum gap to reserve if the leader is braking at
   * maximum
    * @param[in] speed EGO's speed
    * @param[in] leaderSpeed LEADER's speed
    * @param[in] leaderMaxDecel LEADER's max. deceleration rate
    */
  inline SUMOReal getSecureGap(const SUMOReal speed, const SUMOReal leaderSpeed,
                               const SUMOReal leaderMaxDecel) const {
    // The solution approach leaderBrakeGap >= followerBrakeGap is not
    // secure when the follower can brake harder than the leader because the
    // paths may still cross.
    // As a workaround we lower the value of followerDecel which errs on the
    // side of caution
    const SUMOReal followDecel = MIN2(myDecel, leaderMaxDecel);
    return MAX2((SUMOReal)0, brakeGap(speed, followDecel, myHeadwayTime) -
                                 brakeGap(leaderSpeed, leaderMaxDecel, 0));
  }
   ...
   */
  virtual void setMaxAccel(SUMOReal accel) { myAccel = accel; }

  /** @brief Sets a new value for maximum deceleration [m/s^2]
   * @param[in] accel The new deceleration in m/s^2
   */
  virtual void setMaxDecel(SUMOReal decel) { myDecel = decel; }
...
protected:
   ...
   */
  SUMOReal maximumSafeFollowSpeed(SUMOReal gap, SUMOReal predSpeed,
                                  SUMOReal predMaxDecel) const;
